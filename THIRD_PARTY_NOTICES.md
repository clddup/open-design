...
OpenDesign contract packages and `@opendesign/model-gateway` directly use `@sinclair/typebox` 0.34.52 from [TypeBox](https://github.com/sinclairzx81/typebox). It is distributed under the MIT License, Copyright (c) 2017-2026 Haydn Paterson. OpenDesign uses its JSON Schema builders, static TypeScript inference and runtime value checking for process, document, Agent and canonical model-wire boundaries; TypeBox does not grant capabilities or execute model tools.
...
`@opendesign/leafer-engine` directly uses `leafer-editor`, `@leafer-ui/draw`, and `@leafer-in/filter` 2.2.9 from [LeaferJS](https://github.com/leaferjs/leafer-ui) and [leafer-in](https://github.com/leaferjs/leafer-in). They and the integrated official Leafer UI/Editor/Viewport/Resize/TextEditor packages are distributed under the MIT License, Copyright © 2023-present Chao (Leafer) Wan. OpenDesign uses them behind an adapter for Renderer canvas rendering, scene projection, viewport, hit testing, selection, direct manipulation, and bounded OpenDesign-owned image-adjustment filters. OpenDesign project files retain the OpenDesign document format; Leafer private scene serialization is not a persistence or Agent boundary.
...
`@opendesign/desktop` directly uses `image-dimensions` 2.5.1 from [sindresorhus/image-dimensions](https://github.com/sindresorhus/image-dimensions). It is distributed under the MIT License, Copyright (c) Sindre Sorhus. OpenDesign uses its in-memory header parser to read PNG, JPEG, GIF, and WebP dimensions without relying on Electron `nativeImage`, whose cross-platform decoder only guarantees PNG and JPEG support.

`@opendesign/desktop` directly uses `react-markdown` 10.1.0 and `remark-gfm` 4.0.1 from [remarkjs](https://github.com/remarkjs). They are distributed under the MIT License, Copyright © Espen Hovlandsdal, Titus Wormer, and contributors. OpenDesign uses them only to render untrusted Agent text blocks as React elements with raw HTML skipped, remote images disabled, and links kept non-navigating; Markdown output does not receive Electron, file, credential, design-write, or network capabilities.

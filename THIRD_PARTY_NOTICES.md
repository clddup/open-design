...
`@opendesign/geometry-service` directly uses `pathkit-wasm` 1.0.0 from [Google Skia PathKit](https://github.com/google/skia/tree/main/modules/pathkit). It is distributed under the BSD 3-Clause License, Copyright (c) 2018 Google LLC. OpenDesign loads it only through an isolated vector-path provider for bounded SVG path operations; PathKit objects and WASM state do not enter design documents, Renderer state, Agent schemas or public contracts.

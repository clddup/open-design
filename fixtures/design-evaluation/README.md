...
Each scenario fixes the user prompt, target count, model context budget, and non-compensating blind-review criteria. A real packaged OpenDesign run must later provide a validated evidence report containing performance milestones, final captures, the final `.opendesign` document, the initial protocol size and tool surface, and independent Critic results for every final target.

...
export const DESIGN_ARRANGE_TOOL_INPUT_SCHEMA = {
  ...
  properties: {
    ...
    insertionIndex: { type: "integer", minimum: 0, maximum: 4096 },
    autoLayout: {
      anyOf: [
        {
          type: "object",
...
export function isDesignArrangeToolInput(
  ...
  if (action === "reorder-grid-tracks") {
    return (
      safeId(input.frameId) &&
      (input.axis === "rows" || input.axis === "columns") &&
      Array.isArray(input.fromIndices) &&
      input.fromIndices.length > 0 &&
      input.fromIndices.length <= 4_096 &&
      input.fromIndices.every(
        (index) => Number.isInteger(index) && index >= 0 && index <= 4_095,
      ) &&
      Number.isInteger(input.insertionIndex) &&
      Number(input.insertionIndex) >= 0 &&
      Number(input.insertionIndex) <= 4_096 &&
      onlyKeys(input, [
        "action",
        "label",
        "pageId",

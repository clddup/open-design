...
export const DESIGN_AGENT_TOOL_SPECS = [
  ...
  },
  {
    ...
    description:
      "Precisely arrange explicit existing layers in the currently bound Design File using host-computed geometry. It aligns selection bounds, distributes or sets exact spacing, performs deterministic one- or two-dimensional Tidy up, assigns horizontal/vertical constraints to a direct ordinary Frame child, resizes a Frame while resolving constraints, or configures Frame-owned linear Auto Layout v1. Auto Layout v1 supports horizontal/vertical direction, fixed non-negative gap, four-side padding, primary/counter start/center/end alignment, hidden-child exclusion, and automatic reflow after content, hierarchy, visibility, size, order, or parent-size changes. Child positions are always host-derived. Constraints v1 supports left/right/stretch/center/scale and top/bottom/stretch/center/scale on translation-only ordinary Frame children; flow children cannot also use constraints. The host previews the complete change and applies one atomic undoable transaction. Targets are stable Page and layer IDs returned by inspection, never the send-time or live user selection. It rejects locked, missing, stale, out-of-scope, non-invertible, ambiguous, lossy, no-op, and over-limit operations. Hug/fill, wrap, auto gap, baseline, min/max, ignore-layout children, grid, Smart Selection canvas handles, and reflow handles remain separate capabilities.",

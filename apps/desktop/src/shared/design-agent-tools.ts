...
export const DESIGN_AGENT_TOOL_SPECS = [
  ...
  },
  {
    ...
    description:
      "Create the current executable target's real artboard and one or more meaningful editable regions in one rollback-safe call. A reviewed multi-target Delivery Scope is not one executable Plan: declare only the first not-yet-planned confirmed target, finish and verify it, then call this tool again when deliveryStage.nextTarget identifies the next stage. In this same call, provide one concise brief-specific designIntent, target job/layout/spacing, visualSystem, required rasterAssetRoles, and justified reusable semanticObjects; never explain every primitive or emit a separate design essay. Main binds trusted skills, the current target's confirmed brief fidelity, and quality defaults while retaining the complete scope separately. The first slice must visibly prove the thesis and causal signature motif, not draw generic circles/cards/gradients and rationalize them afterward. Materialize the complete current target when it fits the 48 model-content-element budget, otherwise submit the smallest coherent real portion and continue it directly after this commit. Main creates host-owned region Frames, so firstSlice elements must parent content to region IDs and never repeat those IDs. For a requested multi-direction Logo exploration, declare logoExploration and all three concept regions inside that current target before drawing so partial evidence cannot be verified. Use 1-3 semantic stages and at most 48 model-authored content elements total, not per stage. Use inspected Pages, prefixed IDs, parent-local geometry, and exact font faces. Full material tools follow success.",

...
export const DESIGN_AGENT_TOOL_SPECS = [
  ...
  },
  {
    ...
    description:
      "Apply a trusted AI image edit to one inspected Image node. remove-background preserves the foreground subject as a transparent PNG; prompt-edit transforms the whole image with at most one authorized reference; erase-object removes a normalized source-image lasso; isolate-object creates a transparent sibling Image layer using a fresh resultNodeId. pageId, nodeId, expectedAssetId, and area geometry must come from current inspection/capture. Do not provide masks, image bytes, paths, provider, or model settings. The host creates and validates the provider mask, then atomically commits result assets, provenance, source history, and either the existing node update or isolated sibling only if the target is still current. Cancellation, provider failure, invalid output, or stale targets produce no revision.",

...
export const DESIGN_AGENT_TOOL_SPECS = [
  ...
  },
  {
    ...
    description:
      "Precisely arrange explicit existing layers in the currently bound Design File using host-computed geometry. It aligns selection bounds, distributes or sets exact spacing, performs deterministic one- or two-dimensional Tidy up, assigns Constraints v1 to an ordinary Frame child, resizes a Frame while resolving constraints, configures Frame-owned Auto Layout v1, or configures direct flow-child sizing. Auto Layout sizing v1 supports per-axis Frame Fixed/Hug and child Fixed/Fill, deterministic main-axis Fill sharing, cross-axis Fill, hidden-child exclusion, nested convergence, and horizontal Fill + Auto Height text remeasurement. Horizontal Wrap v1 supports greedy rows, independent horizontal/vertical gaps, per-row alignment, Fixed/Hug height, oversized-child overflow, and hidden-child exclusion; it requires Fixed Frame width and rejects visible Fill children. Child geometry is always host-derived. The host previews the complete change and applies one atomic undoable transaction. Targets are stable Page and layer IDs returned by inspection, never the send-time or live user selection. It rejects locked, missing, stale, out-of-scope, non-invertible, ambiguous, lossy, no-op, and over-limit operations. Vertical wrap, Wrap+Fill, auto gap, baseline, min/max, ignore-layout children, grid, Smart Selection canvas handles, and reflow handles remain separate capabilities.",

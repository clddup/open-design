...
export const DESIGN_AGENT_TOOL_SPECS = [
  ...
  },
  {
    ...
    description:
      "Edit one or more existing non-branching editable Vector Networks without asking the model to rewrite vertices, segments, path runs, regions, bounds, transforms, or result layer IDs. set-closed opens or closes one explicit contour; reverse-path reverses one contour while preserving effective closed-region winding; cut-path creates a true break at an inspected vertex or at parameter t on an inspected line/cubic segment; cut-with-line divides supported open or closed contours using node-local coordinates; cut-layers-with-line applies one document-space line across explicit Vector layer IDs and atomically divides every crossed target into host-created editable sibling layers. Closed contours require exactly two transverse crossings and receive real closing connectors; open contours split at every transverse crossing into alternating retained/extracted path runs without connectors, regions, or implicit fill. For one compound fill region with an unambiguous outer loop, holes that remain strictly on one side of the line move unchanged with the sibling that contains them, preserving stable path IDs and effective nonzero winding. Targets are stable Page, node, path, vertex, and segment IDs returned by inspection, never the send-time or live selection. The host resolves each layer's world transform, computes geometry through the same versioned vector-edit service as the human canvas, previews the complete change, and applies one atomic undoable EditorRuntime transaction. Uncrossed targets are unchanged; missing, locked, stale, out-of-scope, non-invertible, invalid, branching, tangent, overlapping, crossed-or-touching compound holes, ambiguous outer loops, and ambiguous closed-contour multi-intersection targets are rejected. Connect/disconnect, branches, crossed-hole boundary stitching, concave closed-contour multi-intersection, flatten, and outline stroke remain separate capabilities and must not be simulated with this tool.",

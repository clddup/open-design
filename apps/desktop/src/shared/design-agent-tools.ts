...
export const DESIGN_AGENT_TOOL_SPECS = [
  ...
  },
  {
    ...
    description:
      "Apply a trusted AI image edit to one inspected Image node. remove-background preserves the foreground subject as a transparent PNG; prompt-edit transforms the whole image with at most one authorized reference; erase-object removes a normalized source-image lasso; isolate-object creates a transparent sibling Image layer using a fresh resultNodeId; expand extends explicit node-local edges and grows the existing Image layer without moving its protected source region. pageId, nodeId, expectedAssetId, lasso, and expansion geometry must come from current inspection/capture. Do not provide masks, image bytes, paths, provider, or model settings. The host creates and validates provider inputs, then atomically commits result assets, provenance, source history, and the exact node/layer change only if the target is still current. Cancellation, provider failure, invalid output, or stale targets produce no revision.",

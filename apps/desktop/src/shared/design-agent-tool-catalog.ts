...
export const DESIGN_AGENT_TOOL_SPECS = [
  {
    ...
    description:
      "For a broad product brief, document attachment, four-or-more deliverables, or a host-required scope review, propose the complete user-visible delivery plan before design planning or canvas writes. Each target must be one independently verifiable screen, flow, visual direction, or asset artboard; do not collapse requested product areas into representative artboards and do not split headings, cards, or decorative regions into targets. Delivery targets are not document Pages: keep a product suite on the current Page and create one Frame/Artboard per target. Include concise required content, exclusions, and assumptions. The user sees the actual target list and must confirm it. Page creation and cross-Page organization are separate explicit lifecycle operations and are never inferred from target count. A denial ends this Run without changing the canvas; submit a revised plan only after a new user message. Small focused requests execute directly and should not call this tool.",
  ...
  },
  {
    ...
    description:
      "Create the current target's real artboard and meaningful editable content in one rollback-safe call. For reviewed multi-target scope, declare only deliveryStage.nextTarget and finish it before the next call. Include concise designIntent, layout/spacing, visualSystem, raster roles, and justified semantic objects; Main binds skills, fidelity, and quality. The content must visibly prove the thesis and signature motif, not retrofit reasons to generic primitives. Complete the target within 48 model-authored elements or submit its smallest coherent portion. Parent content to host-owned region IDs and never recreate them. Multi-direction Logo work must declare three regions, principles, color systems, and evidence together. Use 1-3 stages, 48 elements total, inspected Pages, prefixed IDs, parent-local geometry, and exact fonts. After success, general Runs or explicit existing-artboard edits may expose advanced material tools; a new-design Run remains on its compact continuation surface.",

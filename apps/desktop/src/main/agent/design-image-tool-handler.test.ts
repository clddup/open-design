...
function dependencies(call: ToolCallRequest) {
  ...
  const referenceHost = {
    ...
    materializeImage: vi.fn().mockResolvedValue({
      attachment,
      data: Buffer.from(losslessWebpFixture()).toString("base64"),
      mimeType: "image/webp" as const,
    }),
  };
  const imageGenerationHost = {
    generateImage: vi.fn().mockResolvedValue({
      bytes: losslessWebpFixture(),
...
describe("Design Image Main tool handler", () => {
  ...
  it("rejects invalid generated bytes before creating an attachment", async () => {
    const setup = dependencies({
      toolCallId: "generate_invalid",
      toolName: GENERATE_IMAGE_TOOL_NAME,
      input: { prompt: "A hero image", role: "hero" },
    });
    setup.imageGenerationHost.generateImage.mockResolvedValue({
    ...
    await expect(handleDesignImageTool(setup.input)).rejects.toThrow(
      "Generated image is not a supported raster image",
    );
    expect(setup.attachmentHost.importImageBytes).not.toHaveBeenCalled();
    expect(setup.referenceHost.registerGeneratedImage).not.toHaveBeenCalled();
    expect(setup.execute).not.toHaveBeenCalled();
  });
...
function losslessWebpFixture(): Uint8Array {
  return Buffer.from(

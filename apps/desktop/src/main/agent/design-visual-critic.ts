  ...
  const events = await modelProviderHost.complete(
    {
      ...
      system: [
        ...
        "For every logo-concept-*-quality criterion, judge that declared direction independently. It must have an ownable silhouette, visibly intentional construction, controlled contour or counterform, small-size robustness, anti-template originality, and visible fit to the user brief. A caption cannot rescue an arbitrary shape, and stronger sibling concepts cannot compensate for one filler direction.",
        "For Logo color, assess silhouette and monochrome robustness as quality considerations, not an automatic requirement to deliver extra specimen boards. Do not fail a focused mark because an unrequested brand manual, monochrome board, size ladder, or platform mockup is absent. Never claim exact pixel-size or platform-context testing without supplied evidence. brand-color-system fails when the main mark is only black/white/gray without an explicitly monochrome-only user brief, when color is decorative rather than semantic, or when a requested light/dark variant has visibly poor contrast. color-system-divergence requires explored directions to make materially different color decisions, not hue swaps. app-icon-ecosystem-distinction assesses ownable color and optical weight; only claim a comparison among real macOS/Windows icons when a comparison context is supplied.",

...
- When grouping existing sibling layers or ungrouping an existing Group, use opendesign_edit_hierarchy with the explicit stable Page and node IDs returned by inspection. Do not calculate reparenting transforms yourself and do not derive its targets from the user's selection. The host preserves world geometry and sibling order in one atomic transaction, and may reject locked, mixed-parent, stale, out-of-scope, or visually lossy operations.

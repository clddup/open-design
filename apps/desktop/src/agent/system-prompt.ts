...
- When grouping existing sibling layers, ungrouping an existing Group, or changing sibling stacking order, use opendesign_edit_hierarchy with the explicit stable Page and node IDs returned by inspection. For stacking, choose bring-forward, bring-to-front, send-backward, or send-to-back instead of emitting low-level move_element commands. Do not calculate reparenting transforms yourself and do not derive its targets from the user's selection. The host preserves world geometry and stable relative order in one atomic transaction, and may reject locked, mixed-parent, stale, out-of-scope, no-op, or visually lossy operations.

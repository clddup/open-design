...
export class StaticEditorOverlayController {
  ...
  #syncTransformedOverlays(
    elements: ReadonlyMap<string, LeaferElement>,
    specs: ReadonlyMap<string, ComponentSlotOverlaySpec | SliceOverlaySpec>,
    layer: LeaferGroup,
    dashLength: number,
  ): void {
    ...
    for (const [id, element] of elements) {
      ...
      setTransform(element, relative);
      element.set({
        dashPattern: [dashLength / zoom, 4 / zoom],

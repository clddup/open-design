...
import type { LeaferSmartSelectionSpacingRequest } from "./types.js";
...
export class SmartSelectionOverlayController {
  ...
  readonly #rings = new Map<string, LeaferElement>();
  ...
  syncViewport(): void {
    ...
    for (const ringSpec of plan.rings) {
      this.#rings.get(ringSpec.id)?.set({
  ...
  #createRing(id: string): LeaferElement {
    const ring = new this.#leafer.Ellipse({
      editable: false,
  ...
  #reconcileElements(plan: SmartSelectionOverlayPlan): void {
    ...
    for (const spec of plan.rings) {
      this.#rings.set(spec.id, this.#createRing(spec.id));
    }
  }
  ...
  #syncAppearance(): void {
    ...
    for (const ring of this.#rings.values()) {
      ring.set({ opacity: this.#hovered || this.#drag ? 1 : 0.72 });
    }
  }

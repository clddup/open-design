...
能力清单版本：`1` · 更新日期：2026-08-13 · 文档协议：`1.17.0` · 画布基线：`leafer-editor@2.2.9`
...
- 实现方：DesignDocument 1.17.0 + @opendesign/layout-service Auto Layout contract v5 + EditorRuntime
- 表面：contract=available；runtime=available；human=available；agent=available；render=available；export=degraded
- 证据：自动化 14 项；实机 0 项
- 限制：Auto Layout 支持 Frame 每轴 Fixed/Hug、直属子层每轴 Fixed/Fill、宽高 Min/Max、padding minimum、bounded Fill 重分配、主轴 Auto gap、嵌套收敛，以及水平 Wrap 的贪心分行、逐行独立 Auto gap、隐藏子层排除和超宽子层溢出。交叉轴 Auto gap、垂直 wrap、Wrap+Fill、baseline、ignore-layout child、grid、画布回流手柄、SVG 往返 metadata，以及 macOS/Windows 打包 GUI 实机证据仍未完成。

...
能力清单版本：`1` · 更新日期：2026-08-14 · 文档协议：`1.19.0` · 画布基线：`leafer-editor@2.2.9`
...
响应式布局 Frame，并添加不改变 child 几何且不导出的 Uniform Layout Guide。

- ID：`layout.auto-layout`
- 实现方：DesignDocument 1.19.0 Layout Guide + @opendesign/layout-service Auto Layout contract v6 + EditorRuntime
- 表面：contract=available；runtime=available；human=available；agent=available；render=available；export=degraded
- 证据：自动化 16 项；实机 0 项
- 限制：Uniform Layout Guide 支持 Frame 上稳定的 size、color 与 opacity，并保持在布局、capture 和 export 之外。Columns/Rows guide、guide style 与变量、吸附、Auto Layout Grid 的 track/cell/span/reflow、交叉轴 Auto gap、垂直 wrap、Wrap+Fill、baseline、旋转 absolute child、画布回流手柄、SVG 往返 metadata，以及 macOS/Windows 打包 GUI 实机证据仍未完成。

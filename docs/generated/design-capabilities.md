...
能力清单版本：`1` · 更新日期：2026-08-13 · 文档协议：`1.18.0` · 画布基线：`leafer-editor@2.2.9`
...
- 实现方：DesignDocument 1.18.0 + @opendesign/layout-service Auto Layout contract v6 + EditorRuntime
- 表面：contract=available；runtime=available；human=available；agent=available；render=available；export=degraded
- 证据：自动化 14 项；实机 0 项
- 限制：Auto Layout 支持 Frame 每轴 Fixed/Hug、flow child 每轴 Fixed/Fill、宽高 Min/Max、padding minimum、bounded Fill 重分配、主轴 Auto gap、嵌套收敛、水平 Wrap，以及退出 flow 并使用响应式 constraints 的明确 absolute child。交叉轴 Auto gap、垂直 wrap、Wrap+Fill、baseline、grid、旋转 absolute child、画布回流手柄、SVG 往返 metadata，以及 macOS/Windows 打包 GUI 实机证据仍未完成。

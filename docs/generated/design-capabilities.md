...
- 实现方：@opendesign/import-export-service SVG v1 + EditorRuntime planners + PathKit geometry
- 表面：contract=available；runtime=available；human=unavailable；agent=unavailable；render=unavailable；export=degraded
- 证据：自动化 3 项；实机 0 项
- 限制：EditorRuntime 导入与导出 planner 已提供显式目标、revision、层级、world bounds、viewport、padding、z-order 和 Boolean snapshot 处理；Main 文件桥、人工命令、Agent tool 和打包产品路径仍未完成，因此应用 UI 暂时不可直接使用。

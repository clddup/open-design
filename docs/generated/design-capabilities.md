...
- 实现方：DesignDocument 1.4.0 + PropertiesPanel + Leafer adapter + SVG filter adapter
- 表面：contract=available；runtime=available；human=available；agent=available；render=available；export=degraded
- 证据：自动化 7 项；实机 0 项
- 限制：可编辑 SVG 边界现可保留最多八个普通零 spread 投影、一层 layer blur、效果顺序和效果可见性；内阴影、背景模糊、光晕、阴影 spread/blend、蒙版、复杂 filter graph、Electron 视觉基线、专业取色器和共享颜色样式仍未完成。
...
把受支持的 SVG 结构与基础滤镜效果导入为可编辑 OpenDesign 矢量，并以显式保真报告导出 Path、Vector、基础图形、效果和已解析 Boolean 结果。
...
- 限制：当前子集覆盖 Group、Rectangle、Ellipse、Path/Vector、transform、纯色与线性/径向渐变、居中描边、最多八个普通零 spread 投影和一层 layer blur；效果可见性与顺序可确定性往返，并可直接导入标准单个 feDropShadow/feGaussianBlur filter。文字、图片、内阴影/背景效果、光晕、spread/blend、蒙版、stylesheet、角度渐变、多 paint、复杂 filter graph 与内外描边保真仍不可用。

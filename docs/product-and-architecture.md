...
Agent 参考 Pi/OpenCode 的工程思路：保持核心循环小而透明，以消息、工具、事件和持久会话为基础，通过 provider adapter、skills 和 MCP Client 扩展。生产 Model Gateway 已通过 OpenDesign canonical adapter 使用固定 `@earendil-works/pi-ai 0.84.1`；ADR-0020 还接受固定 `@earendil-works/pi-agent-core 0.84.1` 的 headless `Agent` 分阶段迁移。阶段 1 contract/parity tests 已证明顺序工具循环、逐轮 `transformContext`、非法工具拒绝、三种 API identity 往返，以及 `Pi Agent → Main-proxied ModelGateway → OpenDesign tool → 第二个 Provider turn`；Pi model/message lifecycle 已可投影到现有 `AgentEvent 3.4` 和唯一 Conversation journal writer，并与旧 Runtime 的成功、Provider failure 和非法 tool-use stop 终态对照。生产入口仍使用现有 Runtime；Pi tool events 在 risk、approval、revision 和附件 adapter 完成前会明确失败，之后还必须通过上下文、恢复和双平台门禁。固定版本的 `AgentHarness.prompt()` 仍抛出 `HarnessNotImplemented`，Pi Coding Agent、TUI、文件/shell 工具、凭据和资源发现不进入产品路径。详细裁决见 [ADR-0020](adr/0020-pi-headless-agent-loop-migration.md)。

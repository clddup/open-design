...
- 独立的 `@opendesign/geometry-service` contract v12：根入口提供多层排列与 Tidy up；固定 `pathkit-wasm 1.0.0` 的 `vector-path` 子入口提供受控 PathOps/outline 基础；`editable-vector` 验证稳定 vertex/segment/path/region ID、拓扑、path 与 tight bounds；`vector-edit` 负责非分支 contour 的节点/手柄/point mode、Connect/Disconnect、Open/Close/Reverse、节点 Lasso 所需 bounds、节点仿射 transform，以及点击/拖拽 Cut。节点仿射 transform 同时更新所选节点和附着 Bézier tangent endpoints；Leafer Vector Edit 提供 session-only `Q` Lasso 和同一 Vector 多点 move/八向 resize/rotate box，pointer-up 通过 Runtime 只提交一条事务，Agent 以显式 vertex IDs + node-local matrix 复用同一 planner。Cut 继续支持开放多交点、closed boundary graph、outer+hole stitching、凹形多 component 和多 Vector document-space 原子分割。`DesignDocument 1.36.0`、EditorRuntime、Leafer、Boolean resolver 与受控 SVG metadata v2 消费同一 network 事实；普通第三方 SVG 保留精确 path-data，不猜测 network。path segment selection、跨层统一节点框、Space 中途平移、跨层连接与分支网络、flatten、outline stroke，以及完整像素/双平台产品证据仍未完成，因此 capability 保持 `degraded`。

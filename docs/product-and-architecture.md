...
上述综合清单中的文字部分已由 `DesignDocument 1.29.0` Typography Core v2 与 Text Service v3 取代：旧 `textOverflow: ellipsis` 迁移为独立的 clip + ending truncation，paragraph indent/spacing、case、decoration 与 `maxLines` 已贯通 Inspector、Agent、Leafer、共享 Text Style、Figma interop 和 SVG metadata v4；文档仍保存完整 content，Fixed 文字框 size 仍是权威边界。字体 availability 属于当前 provider，Inspector 与专用 Agent 字体工具通过 `expectedFont` 和稳定 node ID，对精确 family/weight 匹配做一个 revision/undo 的显式替换或 Auto Size 回流。当前文字缺口是富文本 runs/列表、OpenType/variable-font axis、字体二进制 asset/授权与完整 style identity、text-on-path 和确定性跨平台 shaping，见 [ADR-0074](adr/0074-typography-core-v2.md) 与 [ADR-0076](adr/0076-font-availability-and-explicit-reflow.md)。

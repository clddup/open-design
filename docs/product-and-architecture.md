...
- 独立的 `@opendesign/geometry-service` contract v13：根入口提供多层排列与 Tidy up；固定 `pathkit-wasm 1.0.0` 的 `vector-path` 子入口提供受控 PathOps/outline 基础；`editable-vector` 验证稳定 vertex/segment/path/region ID、拓扑、path 与 tight bounds；`vector-edit` 负责非分支 contour 的节点/手柄/point mode、Connect/Disconnect、Open/Close/Reverse、节点/segment selection 所需几何、节点仿射 transform、point/path Delete，以及点击/拖拽 Cut。Leafer Vector Edit 以 session-only `selectedVertexIds/selectedSegmentIds` 提供 `Q` Lasso、路径点击、高亮和同一 Vector 多点变换；删除 segment 会真实断开 contour，删除 vertex 继续重连相邻边，完成动作通过 Runtime 只提交一条事务。Cut 继续支持开放多交点、closed boundary graph、outer+hole stitching、凹形多 component 和多 Vector document-space 原子分割。`DesignDocument 1.36.0`、EditorRuntime、Leafer、Boolean resolver 与受控 SVG metadata v2 消费同一 network 事实；普通第三方 SVG 保留精确 path-data，不猜测 network。跨层统一节点框、Space 中途平移、segment Bend/per-segment appearance、跨层连接与分支网络、flatten、outline stroke，以及完整像素/双平台产品证据仍未完成，因此 capability 保持 `degraded`。

...
上述综合清单中的文字部分已由 `DesignDocument 1.29.0` Typography Core v2 取代：旧 `textOverflow: ellipsis` 迁移为独立的 clip + ending truncation，paragraph indent/spacing、case、decoration 与 `maxLines` 已贯通 Text Service v2、Inspector、Agent、Leafer、共享 Text Style、Figma interop 和 SVG metadata v4；文档仍保存完整 content，Fixed 文字框 size 仍是权威边界。当前文字缺口是富文本 runs/列表、OpenType/variable-font axis、字体 asset/替换与显式 reflow、text-on-path 和确定性跨平台 shaping，见 [ADR-0074](adr/0074-typography-core-v2.md)。
...
生产工具 catalog 采用只影响 Provider 视图的渐进披露。新 Run 首轮只发送七个必要工具；inspection 后只增加 Capability 与 SVG/Raster Export，create-artboard Plan 与首个材料切片继续保持十工具以内的轻量视图。基础 `apply_transaction` 仅包含 Frame/Group/Rectangle/Ellipse/Text、solid paint 和基本 insert/update/move/delete。create Plan 分配的真实空 Frame 不算材料进展，也不展开 catalog；第一笔材料 revision 或 Page lifecycle 写入后恢复完整二十一工具和完整 apply schema，existing-artboard Plan 则在下一轮直接开放专业编辑工具。各视图执行同一原始 tool definition、完整 runtime validator、Main host、EditorRuntime、approval、revision 和 history，不存在 bootstrap executor 或第二份设计状态。工具视图变化会重新计算 Model Profile fixed protocol budget。详见 [ADR-0073](adr/0073-progressive-agent-tool-disclosure-and-bootstrap-apply.md)。

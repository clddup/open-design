...
- OpenDesign 自有的 `DesignDocument 1.4.0`、正式 SVG Path/Vector 外观语义、非破坏图片 placement 与非破坏 Boolean Group、多 Page、事务、preview、单调 revision、diff、history、undo/redo、checkpoint，以及 `1.0.0` / `1.1.0` / `1.2.0` / `1.3.0 → 1.4.0` 迁移。
...
- 独立的 `@opendesign/geometry-service` contract v2：根入口提供确定性排列；隔离的 `vector-path` 子入口固定 `pathkit-wasm 1.0.0`，以短生命周期、纯数据 provider 通过真实 WASM 的 cubic PathOps、孔洞、空结果、simplify、transform、dash、outline stroke、fill rule、bounds、预算和确定性测试。`DesignDocument 1.4.0` 与 EditorRuntime 已增加独立非破坏 Boolean 节点及 planner；递归 resolver 会计算 Rectangle/Ellipse/Path/Vector/嵌套 Boolean 的 fill+stroke 几何，Leafer 按需加载 WASM 并投影稳定 synthetic result，派生 path 不进入文档。人工工具栏/菜单、双平台快捷键、Inspector 切换、解组、源 operand 画布编辑和 `opendesign_edit_hierarchy` Agent typed actions 已复用同一 planner/resolver。`@opendesign/import-export-service` 还建立了固定 XML/transform parser 的 SVG v1 纯 service contract，可导入可编辑 Path/Vector/基础 shape，并把显式解析的 Boolean result 作为标准 path 导出且返回 fidelity report；EditorRuntime 导入 planner 已将候选树转换为显式目标下的单次可预览、可撤销事务，并校验层级、锁定、冲突、引用和命令预算。导出 planner 已从显式 Page/root IDs、base revision、world bounds、paint order、padding 和同 revision Boolean snapshot 构造 0-origin 纯 service request，不读取实时选区。Main 已通过窄 IPC 提供路径不外泄的 `.svg` 原生打开/保存桥，使用 fatal UTF-8、共享字符/字节预算、扩展名校验和同目录临时文件原子写入；人工导入/导出与 Agent 导入/导出已复用同一可取消 worker。Agent 导入使用当前 Run 内容寻址 SVG handle 和显式稳定目标，Main 才能物化 XML；模型不接收 XML、路径或内部 ID 前缀。完整外观与像素/双平台产品证据仍未完成，因此相关 capability 保持 `degraded`。
- SVG v1 当前还可确定性往返圆角 Frame `clipsContent` 与按父容器 child order 分段的 alpha/luminance/outline/clipping sibling masks；受支持的第三方本地 user-space `<mask>/<clipPath>` 会展开为可编辑同级蒙版组。复杂组合 mask graph、Text/Image mask source、objectBoundingBox clip 与真实像素/双平台产品证据仍明确不在已完成范围。
...
Agent Runtime 使用 TypeScript 并运行于 Electron `utilityProcess`。当前 `AgentRequest 3.5` 的 `run.start` 包含 `sessionId`（承载产品 `conversationId`）、`runId`、prompt、model selection、可选的图片/文档/SVG 附件元数据、单个 `documentId`、revision、发送时的选区上下文与独立的单一 `mutationTarget`。选区只帮助模型理解用户当时关注的节点，不授予也不缩小写入范围；Mutation Target 通常固定为发送时的活动 Page，任务开始后 Renderer 的选区和活动页面变化不会改变它。新 `run.start` 必须携带 Mutation Target；历史 `message.user` journal 允许缺省该字段以继续读取 3.2 及更早会话，不会从旧选区反推或补授写权限。Renderer 不得提交 `modelContext`；Main 从所选 Model Profile 解析 `contextWindow/maxOutputTokens` 后再注入 utility process。Main 在转发 Run 前根据当前 Project/Design File 注册并校验 host-bound revision；Conversation journal 只记录历史上下文，不得用历史最大 revision 覆盖当前活动文档事实。文档从持久化版本重新打开或放弃未保存修改后，即使 journal 曾记录更高 revision，也必须接受 Main 已验证的当前基线，后续写冲突仍由 Main 绑定与唯一 `EditorRuntime` 的 `baseRevision` 校验处理。`homeProjectId` 仍由 Main-owned Conversation descriptor 和 Global Task 目标校验承载。目标协议还需要加入完整 Working Set、多 Mutation Targets、能力快照、审批和多目标结果，这些扩展需要后续协议升级和存量会话迁移。
...
`DesignDocument 1.4.0` 已把纯色、线性/径向/角度渐变、图片 Paint、多色标、投影、内阴影、内外光晕、图层/背景模糊、灰度、混合模式、蒙版、高级描边、正式 SVG Path/Vector、Image 节点的 `Stretch / Fit / Fill / Crop` 非破坏 placement，以及保留有序源图层的 Boolean Group 定义为 OpenDesign 公共语义。Boolean Group 持有 operation 与统一外观，不保存 PathKit 派生 path；源层几何与 transform 仍可编辑，组内单层外观由组控制。EditorRuntime 提供原子创建、operation 切换和解组 planner；geometry resolver 与 Leafer synthetic result 已完成派生渲染。人工工具栏/菜单、macOS `⌥⇧U/S/I/E`、Windows `Alt+Shift+U/S/I/E`、Inspector operation 控件和解组入口与 Agent 的 `create-boolean`、`set-boolean-operation`、`ungroup-boolean` actions 全部调用同一 planner；Agent 必须携带检查所得的稳定 Page/node IDs，不读取发送时或实时选区，也不能写入 PathKit 派生 path。人工通过 Enter、双击或图层树进入由当前选区推导的源 operand edit scope，通过 Shift+Enter、Escape 或 Done 返回，Tab 切换 operand；scope 只增加可丢弃轮廓与逐帧 synthetic preview，直接操作松手后仍只提交一个源节点事务。锁定组保持可检查但只读，provider 或 geometry 失败显示编辑源层/重试入口。Crop 保存归一化焦点、缩放、旋转和翻转；原始 asset 不被改写。`@opendesign/image-service` 负责确定性裁剪几何，Leafer adapter 只消费投影。图片继续使用事务化 `DesignAsset`；`put_asset` 与 image node 可以在同一 revision 中应用和撤销，被节点或 image paint 引用的 asset 不得删除。外观规范见 [ADR-0010](adr/0010-open-design-appearance-image-and-reference-semantics.md)，路径与视觉复核规范见 [ADR-0012](adr/0012-formal-path-vector-and-visual-review.md)，图片 placement 规范见 [ADR-0019](adr/0019-versioned-image-placement-and-crop.md)，Boolean 规范见 [ADR-0022](adr/0022-versioned-nondestructive-boolean-groups.md)。
...
Agent 参考 Pi/OpenCode 的工程思路：保持核心循环小而透明，以消息、工具、事件和持久会话为基础，通过 provider adapter、skills 和 MCP Client 扩展。生产 Model Gateway 已通过 OpenDesign canonical adapter 使用固定 `@earendil-works/pi-ai 0.84.1`；ADR-0020 接受固定 `@earendil-works/pi-agent-core 0.84.1` 的 headless `Agent`。阶段 1—3 已完成三种 API identity、`AgentEvent 3.5`、唯一 journal writer、十四个生产工具、plan/review completion guard、取消/失败分流、累计 checkpoint、逐轮 `transformContext` 预算压缩和内容寻址多模态/资源句柄投影。utilityProcess 的唯一生产入口现为 `OpenDesignPiRuntime`；旧自研通用循环及其测试已删除，没有双循环或 fallback，设计事务、权限、凭据、revision、journal 和恢复仍由 OpenDesign 边界拥有。完整生产提示词、十四工具、200K 配置和八轮多模态循环已通过，Pi transcript 不含 inline base64 或 SVG XML；当前待同一 commit 的 macOS/Windows protected package 与 packaged Agent smoke 后关闭迁移。固定版本的 `AgentHarness.prompt()` 仍抛出 `HarnessNotImplemented`，Pi Coding Agent、TUI、文件/shell 工具、凭据和资源发现不进入产品路径。详细裁决见 [ADR-0020](adr/0020-pi-headless-agent-loop-migration.md)。

...
Agent 参考 Pi/OpenCode 的工程思路：保持核心循环小而透明，以消息、工具、事件和持久会话为基础，通过 provider adapter、skills 和 MCP Client 扩展。生产 Model Gateway 已通过 OpenDesign canonical adapter 使用固定 `@earendil-works/pi-ai 0.84.1`；ADR-0020 还接受固定 `@earendil-works/pi-agent-core 0.84.1` 的 headless `Agent` 分阶段迁移。阶段 1 contract/parity tests 已证明顺序工具循环、逐轮 `transformContext`、非法工具拒绝、三种 API identity 往返，以及 model/message lifecycle 到现有 `AgentEvent 3.4` 和唯一 Conversation journal writer 的投影。阶段 2 已接通十二个生产工具的原始 JSON Schema、业务 validation、approval、progress、revision、内容寻址附件元数据和失败回模型，并拒绝 internal host 工具与 TypeBox 私有 schema 标记；plan/review completion guard、拒绝反馈续跑、拒绝上限、max turns/total tokens、取消、pending tool 最终化及 fatal bridge/tool failure 分流均进入可信生命周期。生产入口仍使用现有 Runtime；上下文/恢复和双平台门禁通过后才能一次切换。固定版本的 `AgentHarness.prompt()` 仍抛出 `HarnessNotImplemented`，Pi Coding Agent、TUI、文件/shell 工具、凭据和资源发现不进入产品路径。详细裁决见 [ADR-0020](adr/0020-pi-headless-agent-loop-migration.md)。

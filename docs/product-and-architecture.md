...
- OpenDesign 自有的 `DesignDocument 1.30.0`、正式矢量、文字、响应式布局、Component/Instance/Variant/Slot、Figma-compatible Variables、Shared Styles，以及正式 Slice 与节点有序 Export Settings。Page、节点和设计系统操作共用事务、preview、单调 revision、diff、history、undo/redo、checkpoint；旧文档确定迁移新增 registry/字段，语义不明的非空旧 token 内容明确拒绝而不猜测。
...
- `@opendesign/component-service` contract v4 继续独占 Main/Instance/Variant/Slot 投影；`@opendesign/variable-service` contract v1 在 Component 物化后统一解析 Page→祖先→节点 mode 与 alias chain，再把 BOOLEAN/FLOAT/STRING/COLOR bindings 投影到 Leafer、SVG 和位图。权威节点保留 fallback，mode/value 改动不反写节点。人工 Variables 工作台/Inspector、Agent `opendesign_manage_variables`、inspection、diff/history、autosave 与保存重开共用同一事实。`@opendesign/text-service` 的 Text Layout contract v4、Text Range contract v1 和 Text Run Layout contract v2 分别覆盖单样式 Auto Size/字体状态、UTF-16 range/edit remap，以及未进入文档的 mixed-run wrapping/baseline/glyph outline provider。固定 Leafer provider 保留 Latin/CJK native Text 边界；按需加载的 HarfBuzz 1.4.0 + bidi-js Unicode 13 provider 使用 Main 显式导入、SHA-256 内容寻址的 TTF/OTF/TTC，输出 cluster-aware complex shaping，并由 Adapter 投影为回映唯一原 Text proxy 的 disposable Path。离屏 Page/Frame capture 与 Text/ancestor PNG/JPEG/WebP export 继续从精确 revision projection 重建一次性输出树。正式 rich-text schema/range editor/Agent/Figma/SVG 往返、OpenType/axes、字体随文件打包和双平台视觉门禁仍未完成。`@opendesign/figma-interop` 只用固定 `@figma/plugin-typings 1.133.0` 做公共形状验证；nested Slot、更多 Variable binding、Motion、跨文件 Library 与 Plugin/REST/DTCG adapter 尚未实现。

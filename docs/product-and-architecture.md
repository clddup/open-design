...
- 独立的 `@opendesign/geometry-service` contract v6：根入口提供多层对齐、固定两端均分、明确一维间距，以及按现有 gap 众数与 overlap graph 确定的一维/二维 Tidy up；隔离的 `vector-path` 子入口固定 `pathkit-wasm 1.0.0`，以短生命周期、纯数据 provider 通过真实 WASM 的 cubic PathOps、孔洞、空结果、simplify、transform、dash、outline stroke、fill rule、bounds、预算和确定性测试；`editable-vector` 子入口验证稳定 vertex/segment/path/region ID、拓扑连续性并确定性生成 cubic path 与 tight bounds；`vector-edit` 子入口负责互不连接的非分支 contour 节点多选移动、手柄耦合、point mode、删除、明确 path 的开放/闭合和反转，以及点击节点或 line/cubic 任意位置的真实 Cut。Cut 用重合但拓扑独立的端点表达断口，以 de Casteljau 精确拆分 cubic，并保持未受影响 ID 与有效 region 语义。`DesignDocument 1.10.0` 与 EditorRuntime 保留独立非破坏 Boolean 节点及 planner，并提供正式有向 Line/Arrow、Polygon/Star、editable Vector Network、节点编辑与文字自动尺寸语义；Leafer 用 Arrow/LineEditTool、Polygon、Star、Path、Pen overlay 和互斥的 point/handle/Cut overlay 投影，Boolean resolver 同时消费精确 path-data 与 network 派生 path，并且开放 contour 没有显式 region 时不会被 Canvas/SVG 隐式闭合填充。独立 `@opendesign/text-service` contract v1 接受有界文字样式与 Auto Size 请求，当前由固定 `leafer-editor@2.2.9` provider 返回具体权威尺寸与字体 fallback warning；EditorRuntime 在 insert/update/replace 的同一事务中消费结果，WorkspaceRuntime 向活动、后台和后打开文件同步同一个 provider，不用字符数估算。`@opendesign/import-export-service` 的 SVG v1 纯 service contract 可导入可编辑 Line/Path/Vector/基础 shape，并用受控本地 marker 往返 Line 端点、用逐点校验 metadata 往返零圆角 Polygon/Star、用 schema + topology + rendered `d` 三重校验的 editable-network metadata v2 往返 point mode 与路径拓扑；v1 metadata 继续兼容读取，普通第三方 SVG 仍保留为精确 path-data，不猜测 network。受控 Text metadata v3 往返 `1.10.0` Auto Width/Auto Height/Fixed 与具体尺寸，v2/v1 确定性迁移为 Fixed；普通 SVG 的自动重排和精确 shaping 继续返回 fidelity warning。Boolean result 仍作为标准 path 导出并返回 fidelity report。EditorRuntime planner、Main 路径不外泄的 `.svg` 原生打开/保存桥、人工入口和 Agent run-scoped handle 继续复用同一可取消 worker；模型不接收 XML、路径或内部 ID 前缀。Smart Selection 画布手柄/回流、拖拽穿越 Cut、连接/分支网络、connect/disconnect、flatten、outline stroke、完整外观与像素/双平台产品证据仍未完成，因此相关 capability 保持 `degraded`。

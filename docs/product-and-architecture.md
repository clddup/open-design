...
- OpenDesign 自有的 `DesignDocument 1.13.0`、正式 Line/Arrow、Polygon/Star、精确 SVG path-data / editable Vector Network、文字布局、普通 Frame constraints、线性 Auto Layout v1、非破坏图片 placement/Boolean Group，以及正式 Component/Instance/Override。Page 与节点操作共用事务、preview、单调 revision、diff、history、undo/redo、checkpoint；无 legacy Instance 的 `1.0.0–1.10.0` 及正式 `1.11.0–1.12.0` 可迁移到 `1.13.0`，语义不明的旧占位 Instance 明确拒绝而不猜测。
- `@opendesign/layout-service` contract v1 纯函数求解普通 Frame constraints，以及 Frame 的 horizontal/vertical linear Auto Layout、四边 padding、固定非负 gap 与主轴/交叉轴 start/center/end。EditorRuntime 在每笔事务显式命令与文字测量后 deepest-first 回流，因此增删、隐藏/显示、resize、重排、reparent、父尺寸变化、保存重开与 undo/redo 共用同一权威几何。Inspector、`Shift+A` / `Alt+Shift+A`、Layers、单选 populated Frame resize 和 Agent `opendesign_arrange_layers` 复用同一 planner；流内普通 constraints 自动清除/隐藏，直接画布 geometry 与 Agent 通用旁路明确拒绝且零 revision。hug/fill、wrap、auto gap、baseline、min/max、ignore-layout child、grid、画布回流手柄、SVG metadata 与 breakpoint 尚未实现。

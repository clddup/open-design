...
Agent 的画布生成过程分为正式阶段和可丢弃展示两层。Main 接受的 `DesignPlanToolInput version: 3` 按用户请求声明 `1..N` 个交付 target，但 accepted plan 不再在画布上投影紫色 skeleton：target/frame/region 继续用于校验、账本和 amendment，不能以展示层冒充真实内容。较大的 typed transaction 会按原命令顺序寻找 `EditorRuntime.preview()` 可接受的合法前缀，每个阶段都是正式 revision，并通过同一 history group 在完成后只形成一个 undo；取消会回滚整个组。阶段间只等待两个真实 animation frame，默认不再加入固定 100ms 人为延迟。Renderer 从已提交 Agent `ChangeSet` 派生父级优先的 Leafer 线框/淡入、稳定 node ID 的属性级 tween 和 focus points；Agent cursor 只有在计划 Frame 已真实落入权威文档后才出现，并用固定 typed tool mapping 表达结构、构建、素材、审查、修正和恢复阶段。Provider 自由文本 progress、partial JSON、cursor、临时 tween/reveal 属性和线框均不写入文档。reveal/tween 共用单 RAF，按视口可见量和实际帧时间调整 cadence；同节点连续 Agent revision 从当前显示值 retarget，离屏节点保持最终态。`capture_canvas`、人工直接编辑、手动停止、Run 终态、切页和错误会恢复最终投影并清理展示；Reduced Motion 跳过 reveal/tween/位移动画但保留静态结构状态。普通鼠标 hover 不再绘制无信息灰框，选中与 edit box 仍保留。详见 [ADR-0028](adr/0028-agent-generation-presentation.md) 与 [ADR-0049](adr/0049-trusted-generation-order-and-presentation.md)。

...
- 长 brief 的交付范围使用 Main 选择的用户确认 Delivery Plan：文档附件、长文本、四项以上结构化 brief 或明确四个以上交付在首次设计写入前只披露 scope review 与 recovery inspect；普通聚焦请求继续 direct。用户确认的 deliverable、target 顺序/ID/目标/required content、exclusions、assumptions 与 Page strategy 绑定 executable Plan 和持久 ledger，拒绝则以零画布写入取消当前 Run。确认不是权限授予，Page/File capability 继续独立审批；见 [ADR-0148](adr/0148-user-confirmed-delivery-scope.md)。

...
- 当前 Design File 的真实图片 Assets 面板：从权威文档索引 Image 与 image paint 引用，提供安全预览、搜索、使用次数、跨 Page 定位、只携带稳定 asset ID 的画布拖放、导入、全引用 replace/relink 和零引用删除；Agent 生成图片同时写入零引用 Design File asset，后续 Conversation/Run 可从 inspection 读取稳定 assetId 并直接放置，临时附件/路径/URL 权限仍为 Run-scoped。所有写入继续进入唯一 EditorRuntime，Renderer 不接收路径。字体、跨文件 Library、授权、派生谱系和批量工作台尚未完成。
...
组件解析规范见 [ADR-0045](adr/0045-versioned-components-and-instances.md)、[ADR-0063～0068](adr/0063-figma-compatible-component-properties.md)、[ADR-0106](adr/0106-figma-slot-composition-boundary.md)、[ADR-0107](adr/0107-figma-selection-and-component-targets.md) 与 [ADR-0108](adr/0108-figma-layer-state-and-canvas-hover.md)；Variables Core 见 [ADR-0069](adr/0069-figma-compatible-variables-core.md)。Component Service 先物化 Main/Instance/Variant/Slot，Variable Service 再按实际消费节点的 mode context 解析每条 alias chain，Canvas 与导出因此不会用 Main 所在 Page 的模式错误展开 Instance。Slot 内 nested Instance 的派生选择/编辑和 lock/visibility override 已完成；更多 Variable binding、跨文件 Library 及 Figma Plugin/REST/DTCG 导入导出仍属后续切片；画布矩阵重排已实现。
...
当前演进顺序由 [`roadmap.md`](roadmap.md) 维护：在已经建立 capability manifest 后，先完成固定样张、渲染诊断与实机证据；精确矢量之后优先完成 Component → Instance → Override 主流程，再并行推进图片/文字/海报交付，随后扩展响应式布局、Variants/Variables、导入导出与完整 Agent 权限/MCP。Component/Variant/Slot、派生层统一选择/安全 override 与 Variables Core 同文件垂直切片已完成并保持 `degraded`；更多 Variable binding、跨文件 Library 与外部 adapter 继续按后续切片推进。

...
- `@opendesign/component-service` contract v2 把 Main 的真实 Frame/Group 树作为唯一事实，Instance 只持久化组件引用、壳层变换、Figma-shaped property assignments 与稳定 sourcePath override；当前 revision 的嵌套 subtree 是可丢弃派生投影。`DesignDocument 1.22.0` 已实现 Boolean/Text/Instance-swap definitions、源图层 references、Main 默认值同步、Component Set/VARIANT 唯一组合解析、实例 set/reset、rename/remove 清理及“先选 Variant、再应用成员 typed property、最后 advanced override”的确定优先级。人工 Assets/Layers/Inspector、多选 Combine as variants、专用 Agent `opendesign_manage_components`、inspection、Leafer Canvas、SVG/位图导出、autosave、保存重开和 history 共用同一解析结果。`@opendesign/figma-interop` 只用固定 `@figma/plugin-typings 1.133.0` 做编译期公共 API 形状验证，不进入核心 Runtime；Slot、Set add/remove/dissolve、跨文件 Library、Plugin/REST 导入导出与 Token/Variable 尚未实现。
...
组件解析规范见 [ADR-0045](adr/0045-versioned-components-and-instances.md)，Figma-compatible Component Properties 见 [ADR-0063](adr/0063-figma-compatible-component-properties.md)，Component Set/VARIANT 见 [ADR-0064](adr/0064-figma-compatible-component-sets-and-variants.md)。Component Main 是真实节点树；Component Set 是只含成员 Main 的真实 Frame；Instance 是当前 revision 的引用与可丢弃派生投影。VARIANT 先选择唯一成员，再应用 Boolean/Text/Instance-swap property、嵌套交换和 advanced override；Reset/Detach、Canvas 与导出共用 Component Service v2。Slot、Set add/remove/dissolve、跨文件 Library、Figma Plugin/REST 导入导出和 Token/Variable 不属于已完成切片。

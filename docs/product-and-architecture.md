...
- 多 fill/stroke、渐变、图片 Paint、阴影/光晕/模糊、blend、mask、高级描边和事务化图片 asset 的公共设计语义及属性检查器/Leafer 映射。Image Service contract 7 提供不进入文档/history 的直接裁剪、区域选择与扩图 session，以及进入 Image 节点权威文档的七项非破坏调整、整图提示词编辑、独立背景替换、区域 Erase/Isolate、四边/四角扩图和可信分辨率提升；扩图由 Main 生成受控 Provider canvas 与透明扩展 mask，并把原图保护矩形逐像素复合回结果，分辨率提升则从 source pixels 计算唯一合规目标且不改变节点几何，背景替换只接受新环境描述并由 Main 追加保主体指令。单选 Image 可通过双击或 Inspector 在画布中裁剪、区域编辑、扩图、替换背景或提升分辨率，Inspector/Agent 可调整曝光、对比度、饱和度、色温、色调、高光和阴影，画布、capture 与位图导出共用同一 RGBA 投影。

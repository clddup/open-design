...
- Agent 大型设计事务的文档有效渐进提交与 Leafer 新增节点 reveal：小事务保持原子，大事务只提交通过 `EditorRuntime.preview()` 的连续有效前缀并共享一个撤销组；已提交的 Agent 新节点按父级优先经历短暂线框与淡入。展示状态可丢弃，不进入文档、history、selection、保存或导出；Reduced Motion、Run 终态、切页、错误和截图会收口到当前 revision 的最终投影。
...
Agent 的画布生成过程分为正式阶段和可丢弃展示两层。较大的 typed transaction 会按原命令顺序寻找 `EditorRuntime.preview()` 可接受的最短优选前缀，每个阶段都是合法 revision，并通过同一 history group 在完成后只形成一个 undo；取消会回滚整个组。Renderer 再从已提交 Agent `ChangeSet` 派生父级优先的 Leafer 线框/淡入，不把 Provider partial JSON、动画 opacity 或 overlay 写入文档。`capture_canvas`、Run 终态、切页、错误和 Reduced Motion 都会恢复当前 revision 的最终外观。当前尚未实现 plan skeleton、Agent cursor 和属性级 tween，详见 [ADR-0028](adr/0028-agent-generation-presentation.md)。

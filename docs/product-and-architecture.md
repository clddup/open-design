...
- 多 fill/stroke、渐变、图片 Paint、阴影/光晕/模糊、blend、mask、高级描边和事务化图片 asset 的公共设计语义及属性检查器/Leafer 映射。Image Service contract 5 提供不进入文档/history 的直接裁剪、区域选择与扩图 session，以及进入 Image 节点权威文档的七项非破坏调整、整图提示词编辑、区域 Erase/Isolate 和四边/四角扩图；扩图由 Main 生成受控 Provider canvas 与透明扩展 mask，并把原图保护矩形逐像素复合回结果。单选 Image 可通过双击或 Inspector 在画布中裁剪、区域编辑或扩图，Inspector/Agent 可调整曝光、对比度、饱和度、色温、色调、高光和阴影，画布、capture 与位图导出共用同一 RGBA 投影。
...
参考图分析、确定性图片处理和 AI 图片编辑是三个不同边界。`read_image` 只让多模态模型理解参考图；当前 Image service 已提供 Image 节点的非破坏 placement/crop、adjustment/filter 与 mask，检查器和专用 typed tools 共用可信 planner。AI 图片编辑已包含两条可信路径：去背景要求透明 PNG 与真实透明像素；整图提示词编辑接受一张源图和最多一张人工选择或当前 Run 已授权的参考图。两者都由 Inspector 或 `opendesign_edit_image` 锁定当前 `expectedAssetId`，通过独立 `openai-images` `/images/edits` adapter 执行；只有 Provider 成功且目标仍未过期时，Renderer 才把结果 asset、可选 supporting reference asset、typed derivation 与节点引用作为一笔 revision 提交。取消、失败、无效输出或 stale target 不产生文档 revision，原图、placement、filters、尺寸和节点身份均保留；来源历史可切换、恢复、撤销、保存重开。局部蒙版编辑、扩图、重打光、多参考图与风格统一仍是后续能力。

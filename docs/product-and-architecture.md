...
参考图分析、确定性图片处理和 AI 图片编辑是三个不同边界。`read_image` 只让多模态模型理解参考图；当前 Image service 已提供 Image 节点的非破坏 placement/crop、adjustment/filter 与 mask，检查器和专用 typed tools 共用可信 planner。AI 图片编辑的首个垂直切片是去背景：Inspector 或 `opendesign_edit_image` 锁定当前 `expectedAssetId`，Main 从当前文档边界取得受限源图，`openai-images` adapter 使用 `/images/edits` 请求透明 PNG，并验证真实像素含透明度；只有 Provider 成功且目标仍未过期时，Renderer 才把新 asset、`remove-background` derivation 与节点引用作为一笔 revision 提交。取消、失败、无透明度或 stale target 不产生文档 revision，原图、placement、filters、尺寸和节点身份均保留；来源历史可切换、恢复、撤销、保存重开。局部重绘、扩图、背景替换、重打光与风格统一仍是后续能力。

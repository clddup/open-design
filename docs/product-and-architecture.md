...
`DesignPlan v3` 已在这个单一 Design File/Mutation Target 边界内提供 `1..N` 个交付 target：单个设计只产生一个 target，明确的一套页面、方案或物料按用户列出的交付项逐个产生。Page Mutation Target 只能覆盖该 Page；Document Mutation Target 可以覆盖当前检查结果中的多个 Page。Global Task 持久 `DesignDeliveryLedger v1` 逐项记录 draft、capture、review、refinement 与 final verification，completion guard 在全部 target verified 前自动续跑，中断后的新 Run 可从首个未完成项恢复。该交付账本不授予第二个 Mutation Target，也不等同于跨 Design File/Project 的多目标事务；后一能力仍属于后续协议工作。

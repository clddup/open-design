...
上述综合清单中的文字部分已由 `DesignDocument 1.30.0` Typography Core v2 与 Text Service v4 取代：旧 `textOverflow: ellipsis` 迁移为独立的 clip + ending truncation，paragraph indent/spacing、case、decoration 与 `maxLines` 已贯通 Inspector、Agent、Leafer、共享 Text Style、Figma interop 和 SVG metadata v5；文档仍保存完整 content，Fixed 文字框 size 仍是权威边界。Text/Text Style 正式保存 `fontFamily/fontStyleName/fontWeight/fontSlant`，其中 null style 明确表示 unresolved，不能按 weight 猜造 Figma face。字体 availability 属于当前 provider，Inspector 与专用 Agent 字体工具通过完整 `expectedFont` 和稳定 node ID，对四字段精确匹配做一个 revision/undo 的显式替换或 Auto Size 回流；Figma interop 只从明确 style name 生成 `FontName`。当前文字缺口是富文本 runs/列表、OpenType/variable-font axis、字体二进制 asset/授权、text-on-path 和确定性跨平台 shaping，见 [ADR-0074](adr/0074-typography-core-v2.md)、[ADR-0076](adr/0076-font-availability-and-explicit-reflow.md) 与 [ADR-0077](adr/0077-figma-compatible-font-face-identity.md)。
...
生产工具 catalog 采用只影响 Provider 视图的渐进披露。Main 初始预检成功时，首个 Provider turn 只发送 inspect refresh、Plan、显式 read-image 与 Page 授权/生命周期五个工具；create Plan 分配真实 Frame roots 后才增加基础 `apply_transaction` 和 image generation。预检不可用时保留原七工具 bootstrap，公开 inspection 后只增加 Capability 与 SVG/Raster Export，create-artboard Plan 与首个材料切片继续保持十工具以内的轻量视图。基础 apply 仅包含 Frame/Group/Rectangle/Ellipse/Text、solid paint 和基本 insert/update/move/delete。create Plan 分配的真实空 Frame 不算材料进展；第一笔材料 revision 或 Page lifecycle 写入后恢复完整二十一工具和完整 apply schema，existing-artboard Plan 则在下一轮直接开放专业编辑工具。各视图执行同一原始 tool definition、完整 runtime validator、Main host、EditorRuntime、approval、revision 和 history，不存在 bootstrap executor 或第二份设计状态。工具视图变化会重新计算 Model Profile fixed protocol budget。详见 [ADR-0073](adr/0073-progressive-agent-tool-disclosure-and-bootstrap-apply.md) 与 [ADR-0075](adr/0075-host-inspected-design-orchestrator.md)。

Agent 参考 Pi/OpenCode 的工程思路：保持核心循环小而透明，以消息、工具、事件和持久会话为基础，通过 provider adapter、skills 和 MCP Client 扩展。生产 Model Gateway 已通过 OpenDesign canonical adapter 使用固定 `@earendil-works/pi-ai 0.84.1`；ADR-0020 接受固定 `@earendil-works/pi-agent-core 0.84.1` 的 headless `Agent`。阶段 1—3 已完成三种 API identity、`AgentEvent 3.8`、唯一 journal writer、二十一个生产工具、plan/review completion guard、取消/失败分流、累计 checkpoint、逐轮 `transformContext` 预算压缩和内容寻址多模态/资源句柄投影。utilityProcess 的唯一生产入口现为 `OpenDesignPiRuntime`；旧自研通用循环及其测试已删除，没有双循环或 fallback，设计事务、权限、凭据、revision、journal 和恢复仍由 OpenDesign 边界拥有。七工具 bootstrap、完整展开的二十一个工具、200K 配置和八轮多模态循环均有契约证据，Pi transcript 不含 inline base64、SVG XML 或 raster export bytes；当前待同一 commit 的 macOS/Windows protected package 与 packaged Agent smoke 后关闭迁移。固定版本的 `AgentHarness.prompt()` 仍抛出 `HarnessNotImplemented`，Pi Coding Agent、TUI、文件/shell 工具、凭据和资源发现不进入产品路径。详细裁决见 [ADR-0020](adr/0020-pi-headless-agent-loop-migration.md) 与 [ADR-0073](adr/0073-progressive-agent-tool-disclosure-and-bootstrap-apply.md)。

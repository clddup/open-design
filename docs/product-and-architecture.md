...
截至 2026-08-13，仓库当前具备：

- OpenDesign 自有的 `DesignDocument 1.19.0`、正式 Line/Arrow、Polygon/Star、精确 SVG path-data / editable Vector Network、文字布局、普通 Frame constraints、Auto Layout 轴向 Fixed/Hug/Fill、Horizontal Wrap、Min/Max、Auto gap、Absolute child 与 Frame Uniform Grid guide、非破坏图片 placement/Boolean Group，以及正式 Component/Instance/Override。Page 与节点操作共用事务、preview、单调 revision、diff、history、undo/redo、checkpoint；无 legacy Instance 的 `1.0.0–1.10.0` 及正式 `1.11.0–1.18.0` 可迁移到 `1.19.0`，语义不明的旧占位 Instance 明确拒绝而不猜测。
- `@opendesign/layout-service` contract v6 纯函数求解普通 Frame constraints、linear Auto Layout、Frame Fixed/Hug、直属 flow child Fixed/Fill、Horizontal Wrap、Frame/flow child Min/Max、主轴 Auto gap 与 Absolute child 排除。候选尺寸按 limits clamp，Fill 以 bounded water-filling 重分配，Frame padding 是不可压缩的硬下限；Auto gap 只把非负剩余主轴空间分到相邻 flow child 之间，单项从起点开始，Wrap 按每行独立求解并保留显式 counter gap。Absolute child 保留在 Frame 内但不参与 Hug/Fill/Auto gap/Wrap，父尺寸变化后复用普通 constraints。EditorRuntime 以 deepest-first flow、宽度变化的 Auto Height Text 重测和有界 fingerprint 循环收敛，因此增删、显隐、resize、重排、reparent、父尺寸变化、保存重开与 undo/redo 共用同一权威几何。Inspector、快捷键、Layers、单选 populated Frame resize、absolute child 直接画布操作和 Agent `opendesign_arrange_layers` 复用 planner；flow child 普通 constraints 和直接 geometry、absolute child sizing/limits 与 Agent 通用旁路明确拒绝且零 revision。`DesignDocument 1.19.0` 另提供不参与该求解器的 Frame Uniform Layout Guide，选中 Frame 时由 Leafer editor sky 做不可命中投影，capture/export 不包含。Columns/Rows guide、吸附、Auto Layout Grid 的二维 track/cell/span/reflow、counter-axis Auto gap、Vertical wrap、Wrap+Fill、baseline、画布回流手柄、SVG metadata 与 breakpoint 尚未实现。
...
`DesignPlan v3` 已在这个单一 Design File 边界内提供 `1..N` 个交付 target：单个设计只产生一个 target，明确的一套页面、方案或物料按用户列出的交付项逐个产生。默认 Page Run 只能计划该 Page；获得一次性 Page 结构授权并重新检查后，effective document execution context 可以覆盖当前检查结果中的多个 Page。Main 接受 plan 后，用一次原子、可撤销并立即 autosave 的内部事务为所有 `mode=create` target 创建真实 Page-root Frame；不预建 Region 或垃圾 Group。Global Task 持久 `DesignDeliveryLedger v2` 按 `pending → allocated → drafted → captured → reviewed → refined → verified` 推进：allocated 只证明真实根已进入文档，不能 capture、review、计入 verified 或冒充首稿；首次材料写入才进入 drafted。旧 v1 ledger 在持久读取边界显式迁移。completion guard 在全部 target verified 前自动续跑，中断后的新 Run 从首个未完成项恢复。材料工具只允许写当前 active target，首 target 完成验证后才推进下一 target。Plan amendment 保留已落地 target/Page/root Frame 与 region ID；意图变化把材料目标重置为 drafted。相同 plan 和续跑会从最新 inspection 复核真实 Frame：仅平移继续，resize/rotate/reparent/delete/undo 要求 inspect/amend。该交付账本不授予第二个 Design File/Project Mutation Target，也不等同于跨文件多目标事务；后一能力仍属于后续协议工作。详见 [ADR-0050](adr/0050-allocated-artboards-and-semantic-generation-steps.md)。

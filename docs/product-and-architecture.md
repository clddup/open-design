...
- OpenDesign 自有的 `DesignDocument 1.28.0`、正式矢量、文字、响应式布局、Component/Instance/Variant/Slot、Figma-compatible Variables、Shared Styles，以及正式 Slice 与节点有序 Export Settings。Page、节点和设计系统操作共用事务、preview、单调 revision、diff、history、undo/redo、checkpoint；旧文档确定迁移新增 registry/字段，语义不明的非空旧 token 内容明确拒绝而不猜测。

...
- 多 fill/stroke、渐变、图片 Paint、阴影/光晕/模糊、blend、mask、高级描边和事务化图片 asset 的公共设计语义及属性检查器/Leafer 映射。Image Service contract 3 提供不进入文档/history 的直接裁剪 session，以及进入 Image 节点权威文档的七项非破坏调整；单选 Image 可通过双击或 Inspector 在画布中裁剪，Inspector/Agent 可调整曝光、对比度、饱和度、色温、色调、高光和阴影，画布、capture 与位图导出共用同一 RGBA 投影。

...
- OpenDesign 自有的 `DesignDocument 1.18.0`、正式 Line/Arrow、Polygon/Star、精确 SVG path-data / editable Vector Network、文字布局、普通 Frame constraints、Auto Layout 轴向 Fixed/Hug/Fill、Horizontal Wrap、Min/Max、Auto gap 与 Absolute child、非破坏图片 placement/Boolean Group，以及正式 Component/Instance/Override。Page 与节点操作共用事务、preview、单调 revision、diff、history、undo/redo、checkpoint；无 legacy Instance 的 `1.0.0–1.10.0` 及正式 `1.11.0–1.17.0` 可迁移到 `1.18.0`，语义不明的旧占位 Instance 明确拒绝而不猜测。
- `@opendesign/layout-service` contract v6 纯函数求解普通 Frame constraints、linear Auto Layout、Frame Fixed/Hug、直属 flow child Fixed/Fill、Horizontal Wrap、Frame/flow child Min/Max、主轴 Auto gap 与 Absolute child 排除。候选尺寸按 limits clamp，Fill 以 bounded water-filling 重分配，Frame padding 是不可压缩的硬下限；Auto gap 只把非负剩余主轴空间分到相邻 flow child 之间，单项从起点开始，Wrap 按每行独立求解并保留显式 counter gap。Absolute child 保留在 Frame 内但不参与 Hug/Fill/Auto gap/Wrap，父尺寸变化后复用普通 constraints。EditorRuntime 以 deepest-first flow、宽度变化的 Auto Height Text 重测和有界 fingerprint 循环收敛，因此增删、显隐、resize、重排、reparent、父尺寸变化、保存重开与 undo/redo 共用同一权威几何。Inspector、快捷键、Layers、单选 populated Frame resize、absolute child 直接画布操作和 Agent `opendesign_arrange_layers` 复用 planner；flow child 普通 constraints 和直接 geometry、absolute child sizing/limits 与 Agent 通用旁路明确拒绝且零 revision。counter-axis Auto gap、Vertical wrap、Wrap+Fill、baseline、grid、画布回流手柄、SVG metadata 与 breakpoint 尚未实现。

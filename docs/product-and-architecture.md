...
Agent 参考 Pi/OpenCode 的工程思路：保持核心循环小而透明，以消息、工具、事件和持久会话为基础，通过 provider adapter、skills 和 MCP Client 扩展。生产 Model Gateway 已通过 OpenDesign canonical adapter 使用固定 `@earendil-works/pi-ai 0.84.1`；ADR-0020 还接受固定 `@earendil-works/pi-agent-core 0.84.1` 的 headless `Agent` 分阶段迁移。阶段 1 contract/parity tests 已证明顺序工具循环、非法工具拒绝、三种 API identity 往返，以及 model/message lifecycle 到现有 `AgentEvent 3.4` 和唯一 Conversation journal writer 的投影。阶段 2 已接通十二个生产工具的原始 JSON Schema、业务 validation、approval、progress、revision、失败回模型、plan/review completion guard、全部预算、取消、pending tool 最终化及 fatal bridge/tool failure 分流。阶段 3 已把唯一 journal 恢复、累计 checkpoint、逐 Provider turn `transformContext` 预算/压缩和内容寻址 `image_ref`/`document_ref` 接入 Pi；完整生产提示词、十二工具、200K 配置和八轮多模态循环通过，Pi transcript 不含 inline base64。生产入口仍使用现有 Runtime；下一阶段必须一次切换唯一入口、删除旧循环，并完成 macOS/Windows 同 commit 门禁。固定版本的 `AgentHarness.prompt()` 仍抛出 `HarnessNotImplemented`，Pi Coding Agent、TUI、文件/shell 工具、凭据和资源发现不进入产品路径。详细裁决见 [ADR-0020](adr/0020-pi-headless-agent-loop-migration.md)。

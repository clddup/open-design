...
当前演进顺序由 [`roadmap.md`](roadmap.md) 维护：在已经建立 capability manifest 后，先完成固定样张、渲染诊断与实机证据；精确矢量之后优先完成 Component → Instance → Override 主流程，再并行推进图片/文字/海报交付，随后扩展响应式布局、Variants/变量、导入导出与完整 Agent 权限/MCP。组件优先级提高不改变事实状态：首个垂直切片验收前仍为 `unavailable`。

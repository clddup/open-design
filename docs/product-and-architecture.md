...
组件解析规范见 [ADR-0045](adr/0045-versioned-components-and-instances.md)、[ADR-0063～0068](adr/0063-figma-compatible-component-properties.md) 与 [ADR-0106](adr/0106-figma-slot-composition-boundary.md)；Variables Core 见 [ADR-0069](adr/0069-figma-compatible-variables-core.md)。Component Service 先物化 Main/Instance/Variant/Slot，Variable Service 再按实际消费节点的 mode context 解析每条 alias chain，Canvas 与导出因此不会用 Main 所在 Page 的模式错误展开 Instance。Slot 内 nested Instance 的派生选择/编辑、更多 Variable binding、跨文件 Library 及 Figma Plugin/REST/DTCG 导入导出仍属后续切片；画布矩阵重排已实现。

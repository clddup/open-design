...
当前 `@opendesign/discovery` 已能在隔离边界内发现 user/project/builtin `SKILL.md` 和分层 `AGENTS.md`，处理来源优先级、符号链接逃逸与内容哈希；用户/Project discovery 结果仍未接入生产 Agent、管理 UI、能力审批或审计链，因此用户自定义 Skill/提示词不能描述为已可用。生产设计 Agent 另通过 `@opendesign/design-skills` 自动装配受信任、版本化、内容哈希固定且按 deliverable 路由的内置方法包：UI bundle 约束视觉策略、UX 结构与 capture critic，Graphic bundle 约束主题证据、媒介选择、图形构图与非 UI capture critic。当前 Plan v1 与 Visual Review v1 记录实际引用；它们不替代真实设计工具、不动态读取远端内容，也不扩大写入权限。见 [ADR-0098](adr/0098-versioned-built-in-ui-design-skills.md) 与 [ADR-0104](adr/0104-deliverable-scoped-design-skills-and-evidence-media.md)。

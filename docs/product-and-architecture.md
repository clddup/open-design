...
- `@opendesign/component-service` contract v3 把 Main 的真实 Frame/Group/Slot 树作为唯一事实，Instance 只持久化组件引用、壳层变换、Figma-shaped property assignments、稳定 sourcePath override，以及明确的直接 Slot override subtree；当前 revision 的其余嵌套 subtree 是可丢弃派生投影。`DesignDocument 1.24.0` 已实现 Boolean/Text/Instance-swap/SLOT definitions、源图层 references、Main 默认值同步、Component Set/VARIANT 唯一组合解析、Set add/duplicate/remove/dissolve、显式 property/value 顺序、二维矩阵编辑与 Instance/Slot override 迁移，以及“先选 Variant、再解析 Slot/成员 typed property、最后 advanced override”的确定优先级。Slot settings、Component/Component Set preferred values、指导性 limit warnings、Clear/Reset、detach/remove 物化、空 Slot editor overlay、人工 Inspector、Agent typed actions、Layers、Leafer Canvas、SVG/位图导出、autosave、保存重开和 history 共用同一解析结果。`@opendesign/figma-interop` 只用固定 `@figma/plugin-typings 1.133.0` 做编译期公共 API 形状验证，不进入核心 Runtime；nested Slot、画布矩阵重排、跨文件 Library、Plugin/REST 导入导出与 Token/Variable 尚未实现。
...
组件解析规范见 [ADR-0045](adr/0045-versioned-components-and-instances.md)，Figma-compatible Component Properties 见 [ADR-0063](adr/0063-figma-compatible-component-properties.md)，Component Set/VARIANT 见 [ADR-0064](adr/0064-figma-compatible-component-sets-and-variants.md)，成员生命周期见 [ADR-0065](adr/0065-component-set-membership-lifecycle.md)，二维属性矩阵见 [ADR-0066](adr/0066-figma-compatible-variant-property-matrix.md)，Slot 见 [ADR-0067](adr/0067-figma-compatible-component-slots.md)。Component Main 是真实节点树；Component Set 是只含成员 Main 的真实 Frame；Instance 是当前 revision 的引用、明确 Slot override subtree 与其余可丢弃派生投影。VARIANT 先选择唯一成员，再解析 Slot、Boolean/Text/Instance-swap property、嵌套交换和 advanced override；Set 成员生命周期与二维矩阵、Slot Clear/Reset/Detach、Canvas 与导出共用 Component Service v3。nested Slot、画布矩阵重排、跨文件 Library、Figma Plugin/REST 导入导出和 Token/Variable 不属于已完成切片。

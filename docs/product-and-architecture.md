...
上述综合清单中的文字部分已由 `DesignDocument 1.31.0`、Typography Core v2、Text Range v1 与 Text Run Layout v2 取代：旧 `textOverflow: ellipsis` 迁移为独立 clip + ending truncation，paragraph indent/spacing、case、decoration 与 `maxLines` 保持贯通；正式 runs 使用 UTF-16 `[start,end)`、完整覆盖与相邻合并，`update_text_range_style`、直接内容编辑重映、Auto Size、undo/save/reopen 全部进入唯一 Runtime。Leafer TextEditor 的真实 DOM selection 驱动 Inspector range/Mixed 编辑，Agent 使用同一 typed transaction。生产画布、Agent capture 和 raster export 从 exact revision 派生 native Text fragments 或 HarfBuzz glyph Paths，失败保留权威 Text 并报告 fidelity warning；派生节点不持久化。Figma styled segments 与 SVG metadata v6/styled `<tspan>` 已结构往返，exact face identity 仍不按 weight 猜造。Main 字体二进制继续只经用户显式导入、内容寻址且不向 Renderer 暴露路径。当前文字缺口是列表、逐段 paragraph 与高级 decoration、OpenType/variable-font axis、绑定 range 的 Style 实时传播、字体随 Design File 打包与授权迁移、text-on-path、Unicode 13 之后 bidi 数据及双平台确定性视觉门禁，见 [ADR-0074](adr/0074-typography-core-v2.md)、[ADR-0079](adr/0079-figma-compatible-text-range-core.md)、[ADR-0081](adr/0081-harfbuzz-complex-text-shaping.md) 与 [ADR-0082](adr/0082-figma-compatible-rich-text-runs.md)。

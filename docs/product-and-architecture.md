...
`DesignPlan v3` 已在这个单一 Design File 边界内提供 `1..N` 个交付 target：单个设计只产生一个 target，明确的一套页面、方案或物料按用户列出的交付项逐个产生。默认 Page Run 只能计划该 Page；获得一次性 Page 结构授权并重新检查后，effective document execution context 可以覆盖当前检查结果中的多个 Page。Global Task 持久 `DesignDeliveryLedger v1` 逐项记录 draft、capture、review、refinement 与 final verification，completion guard 在全部 target verified 前自动续跑，中断后的新 Run 可从首个未完成项恢复。Plan 不是首次写入后永久冻结：宿主为每个 Run 维护独立 `planRevision`，相同输入幂等，pending target 可调整；已产生 material write 的 target 必须保留稳定 target/Page/root Frame 和既有 region ID。视觉系统或 target 意图修订会保留真实节点身份并把受影响 target 重新置为 drafted，要求重新完成 capture/review/refinement/verification；Renderer 只从 Main 返回的权威 amended plan 更新过程骨架。首个新画板事务必须带至少一个真实非容器内容层；任何当次新建的计划区域也必须在同一事务带真实内容。纯空 Frame/Group 结构可以作为可丢弃生成 presentation 展示，但不会推进正式 revision 或 ledger。该交付账本不授予第二个 Design File/Project Mutation Target，也不等同于跨文件多目标事务；后一能力仍属于后续协议工作。

...
- OpenDesign 自有的 `DesignDocument 1.12.0`、正式 Line/Arrow、Polygon/Star、精确 SVG path-data / editable Vector Network、文字布局、普通 Frame constraints、非破坏图片 placement/Boolean Group，以及正式 Component/Instance/Override。Page 与节点操作共用事务、preview、单调 revision、diff、history、undo/redo、checkpoint；无 legacy Instance 的 `1.0.0–1.10.0` 及正式 `1.11.0` 可迁移到 `1.12.0`，语义不明的旧占位 Instance 明确拒绝而不猜测。
- `@opendesign/layout-service` contract v1 纯函数求解普通 Frame 直属子层的横向 left/right/stretch/center/scale 与纵向 top/bottom/stretch/center/scale；Inspector、单选 populated Frame 的 Leafer resize 和 Agent `opendesign_arrange_layers` 共用 EditorRuntime 递归 planner，以单事务更新父 Frame 与全部受影响后代。Reparent 离开 Frame 自动清除关系字段，通用 Agent update 不得绕过专用 resize。Auto Layout、hug/fill、wrap、min/max、grid 与 breakpoint 尚未实现。

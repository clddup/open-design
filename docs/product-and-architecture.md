...
- OpenDesign 自有的 `DesignDocument 1.15.0`、正式 Line/Arrow、Polygon/Star、精确 SVG path-data / editable Vector Network、文字布局、普通 Frame constraints、Auto Layout 轴向 Fixed/Hug/Fill 与 Horizontal Wrap、非破坏图片 placement/Boolean Group，以及正式 Component/Instance/Override。Page 与节点操作共用事务、preview、单调 revision、diff、history、undo/redo、checkpoint；无 legacy Instance 的 `1.0.0–1.10.0` 及正式 `1.11.0–1.14.0` 可迁移到 `1.15.0`，语义不明的旧占位 Instance 明确拒绝而不猜测。
- `@opendesign/layout-service` contract v3 纯函数求解普通 Frame constraints、linear Auto Layout、Frame Fixed/Hug、直属子层 Fixed/Fill 和 Horizontal Wrap。Wrap 以 Fixed width 为阈值，支持贪心分行、独立水平/垂直 gap、逐行主轴及行内/整组交叉轴对齐、Fixed/Hug Height、隐藏层排除和超宽单项 overflow；Wrap+Fill 明确拒绝。EditorRuntime 以 deepest-first flow、宽度变化的 Auto Height Text 重测和有界 fingerprint 循环收敛，因此增删、显隐、resize、重排、reparent、父尺寸变化、保存重开与 undo/redo 共用同一权威几何。Inspector、快捷键、Layers、单选 populated Frame resize 和 Agent `opendesign_arrange_layers` 复用 planner；流内普通 constraints 自动清除/隐藏，直接画布 geometry 与 Agent 通用旁路明确拒绝且零 revision。Vertical wrap、auto gap、baseline、min/max、ignore-layout child、grid、padding minimum、画布回流手柄、SVG metadata 与 breakpoint 尚未实现。

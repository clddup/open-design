...
截至 2026-08-14，仓库当前具备：

- OpenDesign 自有的 `DesignDocument 1.25.0`、正式 Line/Arrow、Polygon/Star、精确 SVG path-data / editable Vector Network、文字布局、普通 Frame constraints、Auto Layout 轴向 Fixed/Hug/Fill、Horizontal Wrap、Min/Max、Auto gap、Absolute child、Frame Uniform Grid 与 Columns/Rows guide、非破坏图片 placement/Boolean Group，以及正式 Component/Instance/Override/Component Properties/Component Set/VARIANT Matrix/Slot。Page 与节点操作共用事务、preview、单调 revision、diff、history、undo/redo、checkpoint；无 legacy Instance 的 `1.0.0–1.10.0` 及正式 `1.11.0–1.24.0` 可迁移到 `1.25.0`，语义不明的旧占位 Instance、半套 Slot state 与当前版本损坏的属性顺序明确拒绝而不猜测。
- `@opendesign/layout-service` contract v6 纯函数求解普通 Frame constraints、linear Auto Layout、Frame Fixed/Hug、直属 flow child Fixed/Fill、Horizontal Wrap、Frame/flow child Min/Max、主轴 Auto gap 与 Absolute child 排除。候选尺寸按 limits clamp，Fill 以 bounded water-filling 重分配，Frame padding 是不可压缩的硬下限；Auto gap 只把非负剩余主轴空间分到相邻 flow child 之间，单项从起点开始，Wrap 按每行独立求解并保留显式 counter gap。Absolute child 保留在 Frame 内但不参与 Hug/Fill/Auto gap/Wrap，父尺寸变化后复用普通 constraints。EditorRuntime 以 deepest-first flow、宽度变化的 Auto Height Text 重测和有界 fingerprint 循环收敛，因此增删、显隐、resize、重排、reparent、父尺寸变化、保存重开与 undo/redo 共用同一权威几何。Inspector、快捷键、Layers、单选 populated Frame resize、absolute child 直接画布操作和 Agent `opendesign_arrange_layers` 复用 planner；flow child 普通 constraints 和直接 geometry、absolute child sizing/limits 与 Agent 通用旁路明确拒绝且零 revision。`DesignDocument 1.20.0` 另提供不参与该求解器的 Frame Uniform Layout Guide，选中 Frame 时由 Leafer editor sky 做不可命中投影，capture/export 不包含；Columns/Rows 是裁剪在 Frame 内的半透明色带，也不参与 child 几何。吸附、Auto Layout Grid 的二维 track/cell/span/reflow、counter-axis Auto gap、Vertical wrap、Wrap+Fill、baseline、画布回流手柄、SVG metadata 与 breakpoint 尚未实现。
- `@opendesign/component-service` contract v4 把 Main 的真实 Frame/Group/Slot 树作为唯一事实，Instance 只持久化组件引用、壳层变换、Figma-shaped property assignments、稳定 sourcePath override，以及明确的直接 Slot override subtree；当前 revision 的其余嵌套 subtree 是可丢弃派生投影。`DesignDocument 1.25.0` 已实现 Boolean/Text/Instance-swap/SLOT definitions、源图层 references、Main 默认值同步、Component Set/VARIANT 唯一组合解析、Set add/duplicate/remove/dissolve、Variant property/value 与 ordinary Component Property 显式顺序、二维矩阵编辑及 Instance/Slot override 迁移，以及“先选 Variant、再按 active member 顺序解析 ordinary property、最后 advanced override”的确定优先级。Slot settings、Component/Component Set preferred values、指导性 limit warnings、Clear/Reset、detach/remove 物化、空 Slot editor overlay、人工 Inspector、Agent typed actions、Layers、Leafer Canvas、SVG/位图导出、autosave、保存重开和 history 共用同一解析结果。`@opendesign/figma-interop` 只用固定 `@figma/plugin-typings 1.133.0` 做编译期公共 API 形状验证，不进入核心 Runtime；nested Slot、画布矩阵重排、跨文件 Library、Plugin/REST 导入导出与 Token/Variable 尚未实现。
...
- Agent 大型设计事务的真实根分配、有序语义步骤、组件决策、语义 Agent cursor、Leafer 新增节点 reveal 与既有节点属性 tween：`DesignPlanToolInput version: 4` 在 v3 多 target 结构上增加 LLM 明示的 component/ordinary 语义候选；Main 只验证声明的稳定节点是否落实为 Frame/Group、Component Main 与 linked Instance，不按类别或次数猜测。接受计划后仍立即通过同一 EditorRuntime 原子分配全部 target 的真实 Frame roots；accepted plan 不展示紫色 skeleton，version 2/3 历史计划继续兼容。Main 从 accepted plan 编译计划 Frame 的可信 Page、parent 和局部几何，Provider 不把局部 bounds 重复换算成世界坐标；计划 region 待首个实质事务按全局唯一稳定 ID 连同真实内容创建。真实顶层 Frame 只平移时，cursor 跟随当前 transform，已建立目标内纯新增事务在 Renderer 复核尺寸、轴对齐和祖先链后可安全 rebase 到最新 revision；resize/rotate/reparent/delete/跨 Page 或覆盖既有节点仍要求重新 inspect。cursor 的阶段只来自固定 tool 生命周期，位置跟随已提交 revision 的新增或更新节点 focus point，不消费 Provider 自由文本。`opendesign_apply_transaction.steps` 按语义边界覆盖全部 command，每个成功步骤产生真实 revision 并共享一个撤销组；未提供 steps 时整笔一次提交。新增节点按父级优先经历短暂线框与淡入，稳定匹配节点只插值实际变化的 transform/geometry/paint/effect/text/path 属性，不兼容语义有界 dissolve。reveal/tween 共用单 RAF 并按可见节点与实际帧时间调节节奏；同节点新 revision 从当前显示值 retarget，选区 editBox 同帧刷新。所有展示状态均可丢弃，不进入文档、history、selection、保存或导出；Reduced Motion、人工编辑、手动停止、Run 终态、切页、错误和截图恢复最终投影。Material write/capture/review 另以 observed document revision 建模，pan/zoom/选区/窗口变化不会制造冲突；可自动恢复的门禁错误返回稳定下一步并默认不堆叠红色时间线卡。
...
组件解析规范见 [ADR-0045](adr/0045-versioned-components-and-instances.md)，Figma-compatible Component Properties 见 [ADR-0063](adr/0063-figma-compatible-component-properties.md)，Component Set/VARIANT 见 [ADR-0064](adr/0064-figma-compatible-component-sets-and-variants.md)，成员生命周期见 [ADR-0065](adr/0065-component-set-membership-lifecycle.md)，二维属性矩阵见 [ADR-0066](adr/0066-figma-compatible-variant-property-matrix.md)，Slot 见 [ADR-0067](adr/0067-figma-compatible-component-slots.md)，普通属性顺序见 [ADR-0068](adr/0068-figma-compatible-component-property-order.md)。Component Main 是真实节点树；Component Set 是只含成员 Main 的真实 Frame；Instance 是当前 revision 的引用、明确 Slot override subtree 与其余可丢弃派生投影。VARIANT 先选择唯一成员，再按 active member 的显式顺序解析 Slot、Boolean/Text/Instance-swap property、嵌套交换和 advanced override；Set 成员生命周期与二维矩阵、ordinary property order、Slot Clear/Reset/Detach、Canvas 与导出共用 Component Service v4。nested Slot、画布矩阵重排、跨文件 Library、Figma Plugin/REST 导入导出和 Token/Variable 不属于已完成切片。

...
截至 2026-08-12，仓库当前具备：

- OpenDesign 自有的 `DesignDocument 1.11.0`、正式 Line/Arrow、Polygon/Star、精确 SVG path-data / editable Vector Network、文字布局、非破坏图片 placement/Boolean Group，以及正式 Component/Instance/Override。Page 与节点操作共用事务、preview、单调 revision、diff、history、undo/redo、checkpoint；无 legacy Instance 的 `1.0.0–1.10.0` 可迁移到 `1.11.0`，语义不明的旧占位 Instance 明确拒绝而不猜测。
- `@opendesign/component-service` contract v1 把 Main 的真实 Frame/Group 树作为唯一事实，Instance 只持久化组件引用、壳层变换与稳定 sourcePath override；当前 revision 的嵌套 subtree 是可丢弃派生投影。人工 Assets/Layers/Inspector、专用 Agent `opendesign_manage_components`、Leafer Canvas、SVG/位图导出、autosave、保存重开和 history 共用同一解析结果。Variants、正式 Component Properties、跨文件 Library 与 Token/Variable 尚未实现。
...
这些是跨 Contracts、EditorRuntime、Main host 和专业 service 的产品能力，不是 `@opendesign/leafer-engine` 单包接口清单。当前 `@opendesign/design-capabilities` 固定 `DesignCapabilityManifest v1`：每项能力同时记录 contract、runtime、human、agent、render 与 export 状态，只有必需表面全通且同时具备自动化和实机证据时才允许标记 `available`。组件首个同文件垂直切片为 `degraded`，布局仍明确返回 `unavailable`；静态导出已由 SVG v1 和 Raster Export v1 贯通人工/Agent/Main 共享链，但批量/Slice/PDF/P3 与产品级双平台实机证据未完成，保持 `degraded`，不能从 schema 占位或第三方引擎说明推断支持。
...
Agent 参考 Pi/OpenCode 的工程思路：保持核心循环小而透明，以消息、工具、事件和持久会话为基础，通过 provider adapter、skills 和 MCP Client 扩展。生产 Model Gateway 已通过 OpenDesign canonical adapter 使用固定 `@earendil-works/pi-ai 0.84.1`；ADR-0020 接受固定 `@earendil-works/pi-agent-core 0.84.1` 的 headless `Agent`。阶段 1—3 已完成三种 API identity、`AgentEvent 3.8`、唯一 journal writer、十九个生产工具、plan/review completion guard、取消/失败分流、累计 checkpoint、逐轮 `transformContext` 预算压缩和内容寻址多模态/资源句柄投影。utilityProcess 的唯一生产入口现为 `OpenDesignPiRuntime`；旧自研通用循环及其测试已删除，没有双循环或 fallback，设计事务、权限、凭据、revision、journal 和恢复仍由 OpenDesign 边界拥有。完整生产提示词、十九个工具、200K 配置和八轮多模态循环已通过，Pi transcript 不含 inline base64、SVG XML 或 raster export bytes；当前待同一 commit 的 macOS/Windows protected package 与 packaged Agent smoke 后关闭迁移。固定版本的 `AgentHarness.prompt()` 仍抛出 `HarnessNotImplemented`，Pi Coding Agent、TUI、文件/shell 工具、凭据和资源发现不进入产品路径。详细裁决见 [ADR-0020](adr/0020-pi-headless-agent-loop-migration.md)。
...
当前演进顺序由 [`roadmap.md`](roadmap.md) 维护：在已经建立 capability manifest 后，先完成固定样张、渲染诊断与实机证据；精确矢量之后优先完成 Component → Instance → Override 主流程，再并行推进图片/文字/海报交付，随后扩展响应式布局、Variants/变量、导入导出与完整 Agent 权限/MCP。组件首个同文件垂直切片已完成并保持 `degraded`；Variants、跨文件 Library 与 Token/Variable 仍按后续切片推进。

...
- 图片编辑复用全局图片服务配置，以 `/images/edits` multipart 提交受限内嵌源图。去背景要求透明 PNG 和真实透明像素；整图提示词编辑允许一张可选参考图，并固定源图/参考图输入顺序。Image Service contract 4 另将画布 Image node-local lasso 按 Stretch/Fit/Fill/Crop、rotation 与 flip 转为归一化源图坐标，Main 生成同源尺寸 RGBA PNG mask；Erase 更新当前来源，Isolate 创建保留 transform/size/placement/filters/圆角的新 sibling Image 且不修改原层。Main 解码并验证结果，Renderer 在远端成功后以 `expectedAssetId` 重新检查，只用一笔事务提交结果 asset、supporting mask/reference asset、derivation 和节点变化。Host、mask PNG、Desktop API、Inspector/App、Agent schema、内部只读 bridge 与 Runtime 测试证明 Escape、失败、取消、无效输出或 stale 不产生 revision，成功来源可恢复、保存重开并可一次 Undo。见 ADR-0135。

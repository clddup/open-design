...
- Text Run Layout Service：`contract v2`（native/HarfBuzz 生产投影已接入）
...
当前 `DesignCapabilityManifest v1` 记录 0 项完整可用、21 项降级可用和 1 项不可用能力；没有实机证据的能力不会标记为完整可用。`DesignDocument 1.31.0`、EditorRuntime、Geometry/Image/Text/Layout/Component/Variable/Style service、Leafer adapter、Inspector 和 Agent tools 已打通正式矢量、文字、外观、图片、响应式布局、Component/Variant/Slot、Variables、Shared Styles、视觉复核和单目标 PNG/JPEG/WebP 导出基础路径。Shared Styles 专项证据覆盖 Paint/Text/Effect/Grid registry、按类型顺序、五类节点引用、fallback 保真解绑/删除、Component → Style → Variable 投影、Canvas/SVG/位图、Local Styles 工作台/Inspector、Agent typed tool/inspection、单 revision/undo 与保存重开。Typography Core v2 + Text Layout Service v4 专项证据覆盖段落缩进/间距、大小写、装饰、ending truncation/max-lines、单样式 Text Style、完整原文恢复、四字段 exact face identity、provider-owned 字体三态、文件级精确替换与显式 reflow、专用 Agent 字体工具、SVG metadata v6/v5/v4/v3/v2/v1，以及只从明确 style name 生成 Figma TextStyle/FontName 的映射。Text Range Service v1 已单独验证 Figma-compatible UTF-16 `[start,end)`、完整覆盖、surrogate boundary、相邻合并和直接编辑 bounded-diff 样式重映；Text Run Layout Service v1 与固定 Leafer provider 进一步验证 mixed face/size/fill、Auto Width / Auto Height / Fixed、跨 run character/word/CJK wrapping、段落几何、grapheme 保守边界、每行 baseline、缺失字体 warning 和 unsupported/measurement-failed 拒绝。Leafer 已建立“原 Text edit proxy + 同父级原生 synthetic Text fragments”的精确 revision 投影：fragment hit 会立即规范化为原 Text selection/chrome，双击只打开原 proxy，编辑期间 fragments 隐藏，成功 revision、Escape、锁定、切页和 dispose 都有确定性恢复；synthetic ID 不进入 Renderer selection 或文档事实。Page/Frame `capture_canvas` 与 Text/ancestor PNG/JPEG/WebP export 现会在目标包含 fragments 时从冻结 scene spec 和精确 revision projection 重建一次性输出树，覆盖编辑中输出独立性、source-local transform、paint order、普通目标快速路径、失败拒绝和销毁清理。权威 rich-text runs、真实 DOM range Inspector、Agent typed transaction、Figma styled segments、SVG v6/styled tspan，以及 native→HarfBuzz 复杂脚本生产投影现已接入同一 revision 链。真实字体跨平台栅格、连接/分支 network、Auto Layout Grid、nested Slot、列表/逐段 paragraph、OpenType、variable axes、字体随文件打包与授权迁移、更多 Variable/Style binding、跨文件 Library/publishing、DTCG/REST/Plugin adapter、像素基线和双平台 GUI 仍未验收，因此相关能力保持 `degraded`。

仓库当前已有独立 `@opendesign/geometry-service` contract v10：确定性排列已进入产品链，隔离的 Skia PathKit provider 已建立路径布尔、simplify、transform、dash 和 outline stroke 的底层计算边界；editable-vector 与 vector-edit 子入口提供稳定 ID 拓扑、节点/手柄、点击 Cut，以及开放/闭合 contour、compound holes、穿孔和闭合凹形多交点的有限线拖拽 Cut。EditorRuntime 在其上提供 document-space 多目标编排、逐层 transform inverse、稳定 sibling 插入与原子多层 Cut。`DesignDocument 1.31.0` 与 EditorRuntime 建立非破坏 Boolean Group、正式 LineNode/PolygonNode/StarNode/Vector Network、持久 point mode，以及 Text Auto Size、wrapping、overflow、Typography Core v2 和 Figma-compatible exact face identity 的独立节点语义、迁移、持久化、planner 与 undo/redo；`path` 与 `network` 严格互斥。独立 `@opendesign/text-service` contract v4 由固定 Leafer provider 提供 Auto Width/Auto Height 测量、具体 size、ending truncation/max-lines、font fallback warning 与 provider-owned `available/missing/unknown` inspection；layout、availability、cache 与 replacement descriptor 均使用 family/style-name/weight/slant，WorkspaceRuntime 把同一 provider 同步给活动、后台和后打开文件。公共 `reflow_text` 保留完整 expected face 竞态门禁，原子替换四字段精确匹配节点并只重测 Auto Size；人工 Inspector 与专用 `opendesign_manage_fonts` 复用同一 revision/history。列表、逐段 paragraph/高级 decoration、OpenType、variable axes、字体随文件打包/授权迁移、text-on-path 与新版 bidi 仍未实现。递归 Boolean resolver 处理正式几何和嵌套 Boolean，Leafer synthetic result 不持久化 provider 派生 path；人工 Pen/Vector 与 Agent typed vector tool 消费同一种 editable network，flatten 和 outline stroke 产品命令仍未完成。独立 `@opendesign/import-export-service` 安全 SVG v1 继续由 EditorRuntime planner、Main 文件桥、人工入口和 Agent handle 复用；editable network 通过有界 metadata v2 往返 point mode 与 Cut 拓扑，Text 通过标准 `<text>/<tspan>` 和受控 metadata v6 往返 resize mode、具体 size、换行、溢出、Typography Core v2 与完整 face identity，v4/v3/v2/v1 继续迁移读取。没有 metadata 的外部 SVG path 保持精确 path-data，不猜测 network；普通第三方 Text 继续拒绝。图片、复杂 effects/combined mask graph、angular gradient、多 paint、outline text/stroke 保真与双平台打包 smoke 仍未完成，因此 SVG capability 保持 `degraded`。仓库已有独立 `@opendesign/layout-service` contract v6；Component Set/VARIANT/Slot、Variables Core、Shared Styles 与单样式 Typography Core v2 已进入正式文档和事务。nested Slot、更多 Variable binding、跨文件 Library 和 DTCG/REST/Plugin adapter 仍未实现。`@opendesign/image-service` 当前提供非破坏 placement/crop 几何，人工 UI 与 Agent 已可替换来源；AI 局部重绘、扩图、背景替换、重打光、风格统一和派生 asset 来源关系仍明确标记为不可用。

...
- 图片编辑复用全局图片服务配置，以 `/images/edits` multipart 提交受限内嵌源图。去背景要求透明 PNG 和真实透明像素；整图提示词编辑允许一张可选参考图，并固定源图/参考图输入顺序。Image Service contract 5 将 Image node-local lasso 按 Stretch/Fit/Fill/Crop、rotation 与 flip 转为归一化源图坐标，Main 生成同源尺寸 RGBA PNG mask；Erase 更新当前来源，Isolate 创建保留 transform/size/placement/filters/圆角的新 sibling Image 且不修改原层。独立 Expand session 使用四边/四角 handle 与比例 preset，Main 把当前 placement 投影到 16px 对齐 Provider canvas、只开放新增边界，并在输出后逐像素覆盖原图保护矩形；专用 Runtime transaction 同步 source、size、transform 与 Stretch placement。Main 解码并验证结果，Renderer 在远端成功后以 `expectedAssetId` 和当前几何重新检查，只用一笔事务提交结果 asset、supporting mask/reference asset、derivation 和节点变化。Host、mask/raster、Desktop API、Inspector/App、Agent schema、内部只读 bridge 与 Runtime 测试证明 Escape、失败、取消、输出漂移或 stale 不产生 revision，普通 derive 不能旁路，成功来源可恢复、保存重开并可一次 Undo。见 ADR-0135 与 ADR-0136。

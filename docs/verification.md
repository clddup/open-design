...
- 图片编辑复用全局图片服务配置，以 `/images/edits` multipart 提交受限内嵌源图。去背景要求透明 PNG 和真实透明像素；整图提示词编辑允许一张可选参考图，并固定源图/参考图输入顺序。Main 解码并验证结果，Renderer 在远端成功后以 `expectedAssetId` 重新检查，只用一笔事务提交结果 asset、可选 supporting reference asset、prompt/reference derivation 和节点引用。Host、Desktop API、Inspector/App、Agent schema、内部只读 bridge 与 Runtime 测试证明失败、取消或 stale 不产生 revision，成功保留 placement/filters/圆角，来源可恢复、保存重开并可一次 Undo。

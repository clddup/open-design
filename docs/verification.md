...
- 去背景编辑复用全局图片服务配置，以 `/images/edits` multipart 提交受限内嵌源图并要求透明 PNG；Host 测试覆盖鉴权、字段、输出格式、大小与取消。Main 进一步解码尺寸和透明像素，Renderer 在远端成功后以 `expectedAssetId` 重新验证，只用一笔事务提交派生 asset、`remove-background` 来源边和节点引用。Inspector/App、Agent schema、内部只读 bridge 与 Runtime 测试证明失败、取消或 stale 不产生 revision，成功保留 placement/filters/圆角，来源可恢复并可一次 Undo。

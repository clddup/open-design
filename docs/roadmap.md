...
- [x] Design Plan 从“首次写入后永久冻结”改为宿主版本化修订：相同 plan 幂等返回，未开始 target 可新增/删除/修改；已落地 target 必须保留稳定 `targetId + Page + root Frame` 与既有 region ID，可修订视觉系统、目标说明和后续步骤。影响已落地目标的 amendment 会把它重新置为 drafted 并要求 capture/review/refinement/verification，权威 amended plan 同步更新画布过程骨架；模型口头完成仍不可信。
...
- [ ] 以“首个真实可用 target 时间”而不只是整套完成时间建立 Agent 生成性能切片：先给 Provider turn、typed tool、progressive apply、两帧等待/固定 delay、capture/review/autosave 分阶段打点，再决定是否引入 `allocated` ledger 状态。候选方案是多目标 plan 后用一次可撤销事务只分配真实稳定 Frame roots，立即完成首个 target，再逐项推进；空 Frame 不能冒充 drafted/verified，内部不预建垃圾 Group。后续真实过程只展示语义事务与已提交 revision，cursor 必须跟随当前 active target，skeleton 只作短时待设计提示。此项会改变 history、恢复、completion guard 和并发 rebase，必须以测量和独立 ADR 验收，不能在本次错误修复中靠删除 delay 或增加动画冒充提速。
  - [x] 建立不含 Prompt、设计正文或完整工具参数的 `design_generation_performance_v1` Run 级测量：Main 汇总 Provider 首事件/首内容/总时长与重试、typed tool 往返、Renderer 首确认和 accepted/applying/capturing/persisting 阶段；Renderer 另返回 progressive apply 的实际 canvas wait 次数/总时长和其中配置的固定 delay 总量，因此 capture、autosave 与两帧等待不再混成一个不可解释的总耗时。里程碑记录 `T_plan/T1/T2/T_all`；现有账本没有真实根分配状态，所以 `T0` 明确为 `null + no-allocated-ledger-state`。`1/4/12` target 确定性场景验证指标语义与有界聚合，但不冒充打包产品的实测性能样本。
...
- [x] 将单目标 plan/review 门禁扩展为持久化结构化交付账本：`DesignPlan v3` 按用户实际需求声明 `1..N` 个交付 target；单个设计只建立一个 target，明确的一套页面/方案/物料逐项建立 target，不擅自扩张或折叠。每项绑定稳定 Page/Frame/root ID，状态按 `pending → drafted → captured → reviewed → refined → verified` 推进；宿主以权威文档、精确 revision、两次 capture/review/refinement 证据验收，不信任模型口头“完成”。create target 另验证计划 region 的直属容器和非空实际内容子树，existing target 则验证真实画板后代材料内容，不把逻辑 region 强加给既有层级。任一必需 target 未完成时 completion guard 在同 Run 自动续跑，持久任务可在中断后的新 Run 从首个未完成项恢复，UI 显示 `N/M verified`；超大工具结果仍单独保留有界账本摘要。Provider 超时后由 Main 自动创建新 Run 尚未实现，继续由下一条用户消息触发恢复。
  ...
  - [ ] 将 `N/M verified` 明确降为事实投影而非“任务不会结束”的解决方案，并实现 Main-owned durable continuation scheduler：Provider timeout、单 Run turn/tool/output 上限或可恢复工具停滞时，从持久 target checkpoint 自动轮换到新 Run，保留稳定 target/Page/Frame ID 与已提交 revision；按失败 fingerprint 有界重试，用户取消、真实不可恢复错误和 `needs-attention` 才停止。首个 target 优先和后续 target 连续推进必须与上面的 `T1/T2/T_all`、真实语义事务 Timeline 一起验收，不能再依赖用户发送“继续”。

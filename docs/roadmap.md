...
- [x] 在同一 Run 的每个 Provider turn 前重新预算；旧 assistant/tool 段超限时生成临时有界 checkpoint，保留当前用户原文和最近完整 tool call/result 段。完整生产 system prompt、二十一个工具、200K Model Profile 与八轮多模态工具循环已证明第八轮会压缩后继续；结构化工具结果同时具有单字段和整体投影上限，原始 journal 不删除。
...
- 继续按垂直切片迁移专业基础文档版本；`DesignDocument 1.27.0` 已在响应式布局、Layout Guide、Component/Variant/Slot 与 Variables 基础上增加 Figma-compatible Paint/Text/Effect/Grid Shared Styles、按类型顺序、五类节点引用和 fallback 保真解绑，后续仍需分支网络、Slice、Auto Layout Grid、Text/Font rich typography、图片 adjustments、nested Slot、画布矩阵重排、跨文件 Library/publishing、更多 style/variable binding 和 export settings。

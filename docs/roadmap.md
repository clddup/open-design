...
- 继续按垂直切片迁移专业基础文档版本；`DesignDocument 1.26.0` 已在响应式布局、Layout Guide、Component/Variant/Slot 基础上增加 Figma-compatible Variable Collection、Mode、六类 typed value、alias、scope/code syntax、核心 binding 与 mode inheritance，后续仍需分支网络、Slice、Auto Layout Grid、Text/Font rich typography、图片 adjustments、nested Slot、画布矩阵重排、跨文件 Library、更多 style/variable binding 和 export settings。

...
- [ ] 将 Renderer 历史巨型全局 `styles.css` 按组件垂直切片迁移为 `Component.module.scss`：CSS Modules 负责作用域，固定 Dart Sass 负责编译期嵌套/复用，design tokens/reset/应用 shell 保持全局；AssetsPanel 与 Canvas 新增状态已先行迁出。不得拆成多份仍互相覆盖的全局 SCSS 冒充模块化，也不并存第二套运行时 CSS-in-JS。见 ADR-0033。

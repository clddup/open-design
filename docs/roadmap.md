...
- [ ] 以“首个真实可用 target 时间”而不只是整套完成时间建立 Agent 生成性能切片：真实根分配与语义步骤垂直切片已完成，macOS/Windows 打包产品的固定 `1/4/12` target 性能与交互 smoke 尚未完成，因此总项保持开放。见 ADR-0050。
  ...
  - [x] 将高置信空白 Page 新建设计进一步收口为专用 compact first-slice surface：首轮只暴露 `opendesign_generate_first_slice + inspect(recovery)`，独立约 4.5 KB prompt；模型一次声明 Plan、全部真实 Frame roots 和第一 target 的真实语义切片，Main 编译为既有 Plan v4 与 canonical transaction，并把 allocation + slice 作为一个 rollback-safe history group执行。成功后按真实 committed revision 推进 `allocated/drafted` 并恢复 general expanded 工具；附件、selection、continuation、已有内容和 Page lifecycle 保持通用路径。固定 prompt + tool surface 从约 73 KB 降至约 16.5 KB（约 `-77%`），不把静态降幅冒充打包产品 T1。见 ADR-0080。

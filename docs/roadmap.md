...
- 扩展 Text/Font service，支持字体二进制 asset/授权/内容寻址加载、富文本 runs、逐段样式、列表、OpenType/variable font axis、text-on-path 和确定性跨平台 shaping。macOS 与 Windows 必须产生明确兼容结果或 fidelity warning。
  ...
  - [ ] 继续完成 synthetic child 实际 hit → original selection、直接编辑 proxy 打开/关闭、capture/export 和专业复杂脚本 shaping provider；全部通过后再共同升级文档 schema、Runtime、Inspector、Agent 与 Figma/SVG 往返，禁止 schema-only runs。

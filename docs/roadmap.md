...
- [ ] 将 Renderer 历史巨型全局 `styles.css` 按组件垂直切片迁移为 `Component.module.scss`：CSS Modules 负责作用域，固定 Dart Sass 负责编译期嵌套/复用，design tokens/reset/应用 shell 保持全局；AssetsPanel、Canvas 新增状态、完整 LeftSidebar、PropertiesPanel、UtilityDock、Agent Timeline/Composer、DiagnosticNotifications、独立 Statusbar、DesignFileTabs、共享 HomeTitlebar/HomeSurface、WorkspaceHome/ProjectHome、Settings Page/Forms，以及按职责分离的 Titlebar/Toolbar/WindowControls 已经迁出，全局入口从 3562 行降至 412 行。剩余范围是 Canvas 既有 chrome、App shell/跨组件布局契约及无引用历史样张规则审计。SettingsPage 的 Provider/Image React state 后续只能随对应业务边界提取，不能为缩短文件建立第二份 draft；PropertiesPanel 的 Paint/Effect/Image/Export 与 AgentTimeline 的 timeline/composer 亦同。不得拆成多份仍互相覆盖的全局 SCSS 冒充模块化，也不并存第二套运行时 CSS-in-JS。见 ADR-0033。

...
- [ ] 保留 `EditorRuntime` invariant 的 `commandId / nodeId / path / message`，让 Renderer/Main/Agent journal/诊断 UI 共同返回结构化、可复制的可恢复 tool result；模型必须重新 inspect 后修正，宿主对相同 fingerprint 的盲重试设上限，只有不存在目标、越权、协议损坏等运行前置失败才终止 Run。

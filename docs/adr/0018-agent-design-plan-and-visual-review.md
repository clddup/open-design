...
- Main coordinator 测试覆盖无计划拒绝、计划 Page、首个 Frame 的位置/尺寸、区域根的类型/直属层级/bounds、嵌套图层、根层散落、图片 role、单图用户证据、Page/Frame capture target、material/capture/review revision 顺序、baseline/重复/过期 capture 拒绝、截图后 review 冻结及终态清理。

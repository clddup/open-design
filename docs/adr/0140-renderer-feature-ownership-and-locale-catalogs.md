...
第五个切片将 Canvas workspace 生命周期迁入独立 controller：viewport zoom/fit 始终在操作时读取当前 `EditorRuntime`，以 viewport 中心为缩放锚点，并从当前 Page/selection 权威几何计算 fit；图层 hover、文字范围/编辑样式、图片 crop/area/expand 入口和 Leafer text provider epoch 都是可丢弃 session bridge，不进入文档或 history。Figma 风格工具、选择、层级、布尔、蒙版、图层状态、缩放与面板快捷键由同一最新 action context 路由；文件切换会解绑旧 Runtime，输入/组合事件与非 Editor 视图不会触发后台画布操作。Canvas host、图片 area/expand overlay、selection actions、hover contract、Leafer transaction bridge 与 responsive resize 现归同一 `features/canvas` owner，并通过公开 entry 交给 `editor-workbench` 组合；Canvas、Editor 与 Workbench 不再反向导入组合层。Canvas 内部 Enter/Tab/Escape 编辑 session 继续在 Canvas host 上优先处理并阻止冒泡，不建立第二套 selection。

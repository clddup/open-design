...
这不等于 Pen、节点编辑或完整 Boolean 产品工作流已经可用。ADR-0022 的 contract/runtime/render、基础人工 UI、源 operand 画布编辑与 Agent tool 表面已有实现，Boolean capability 仍为 `degraded`；SVG 往返、像素基线和双平台打包产品证据未完成前仍不能描述为完整可用。

## 后续门禁

1. 在 macOS/Windows 原生 CI 对同一 PathOps corpus 校验结构结果、耗时、峰值内存和 WASM 加载。
2. 为已完成的非破坏 Boolean planner、派生渲染和源层编辑补齐像素基线、SVG 往返与双平台产品 smoke。

...
当前范围只覆盖同 Project 的 Component/Variant 发布与消费。Styles/Variables 跨文件绑定、Workspace 聚合、远端团队 Library 和发布权限不在本次实现中。
...
- Components/Variants 的协议、runtime、projection、Main 持久化和 Assets/Library 用户流程共用同一 catalog/release；Styles/Variables 跨文件绑定必须继续复用该边界，不得另建私有缓存。

...
Agent Runtime 使用 TypeScript，并由 Electron Main 通过 `utilityProcess` 启动和监督。核心循环参考 Pi/OpenCode 的小内核、事件化会话、工具驱动和可扩展 provider 思路，但只借鉴公开的架构原则；任何代码复用都必须单独完成来源与许可证审查。

...
4. 当前只接受 self-contained selection：后代 background blur、会穿透选择边界的 blend/effect blend、没有包含完整 sibling 作用域的根级 mask，以及单根无法由图片 alpha 保真的 outline/clipping geometry mask 明确失败。不得为了“成功”而猜测未选背景像素、把几何 mask 退化为矩形或产生双重混合。
...
- background blur、依赖未选 backdrop 的 blend/effect blend、真实 Figma 像素 baseline 和 macOS/Windows 打包交互证据仍是后续缺口，因此总体 capability 继续为 `degraded`。

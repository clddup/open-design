...
步骤 8 的 Leafer 阶段已开始但未完成：`ImageCropController` 完整持有 crop session、overlay、pointer/viewport、zoom/reset、键盘完成/取消、stale revision、commit/reject restore、状态发布与 dispose；`BoxDrawController` 完整持有创建容器解析、screen/local 坐标、preview、drag/click 几何、Shift/Alt、创建请求、Escape/tool switch/stale parent/revision、拒绝恢复与 dispose；`PenToolController` 完整持有 document/Page/revision scope、parent 解析、click/drag Bézier draft、zoom-stable preview、闭合候选、键盘编辑、tool-switch 延迟提交、network 规范化、拒绝恢复与 dispose；`BoxSelectController` 完整持有初始 selection snapshot、screen/selector 坐标、pointer-up hit 重算、Shift toggle、最小拖拽阈值、Escape/pointer-cancel 恢复、tool/scope/revision 与 dispose；`DirectTransformController` 完整持有 selection/subtree snapshot、move/resize/rotate/skew 分类、Line endpoint 规范化、Boolean preview 调度、no-op、单次 typed commit、Escape/tool/selection/stale/reject/dispose 恢复；`TextEditDomController` 组合唯一 `TextRunEditController` 并完整持有 edit DOM identity、attach/detach、UTF-16 selection、IME composition、typing-style marker、styled span 原位更新、自动列表与立即 Undo、列表快捷键、错误取消和 listener 清理；`VectorEditController` 完整持有多节点 scope、point/segment selection、anchor/handle/selection overlay、lasso、vertex/handle drag、document-space move/resize/rotate、Space reposition、point mode、path/line Cut、删除、键盘退出、拒绝恢复、stale revision 与 dispose。跨 Page/Design Document、stale parent 或不连续 revision 会清理临时 draft/selection/transform/text/vector session，不能在新同步后的 Runtime 提交旧交互结果；stale transform 与 Vector drag 会主动恢复权威 projection，不留下只存在于 Leafer 的临时几何。adapter 只路由 Leafer/InnerEditor 事件并注入当前 projection/element/tool、Boolean preview 与 transaction callback。pointer event 归一化、Text DOM helper 与 Vector interaction 已进入各自内部 owner，所有临时投影、DOM 内容与 marker 仍未越过 `@opendesign/leafer-engine` 或进入文档/history。其余 reconcile、generation presentation、scheduler 和 geometry/resource lifecycle 仍留在 adapter，SVG 与 release inventory 也保持开放。

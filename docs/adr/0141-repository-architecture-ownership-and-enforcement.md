...
步骤 8 的 Leafer 阶段已开始但未完成：`ImageCropController` 完整持有 crop session、overlay、pointer/viewport、zoom/reset、键盘完成/取消、stale revision、commit/reject restore、状态发布与 dispose；`BoxDrawController` 完整持有创建容器解析、screen/local 坐标、preview、drag/click 几何、Shift/Alt、创建请求、Escape/tool switch/stale parent/revision、拒绝恢复与 dispose。adapter 只路由事件并注入当前 projection/element/tool 与 transaction callback；Box Select 保持独立选择 owner。pointer event 归一化进入共享内部模块，所有临时投影仍未越过 `@opendesign/leafer-engine` 或进入文档/history。其余 Vector/Pen/Text interaction、reconcile、generation presentation、scheduler 和 geometry/resource lifecycle 仍留在 adapter，SVG 与 release inventory 也保持开放。

...
步骤 8 的 Leafer 阶段已开始但未完成：Image Crop、Box Draw、Pen、Box Select、直接变换、Text DOM 与 Vector Edit 的完整 session、preview/overlay、pointer/key、scope/revision、提交/拒绝恢复、取消和 dispose 已分别迁入 `ImageCropController`、`BoxDrawController`、`PenToolController`、`BoxSelectController`、`DirectTransformController`、组合唯一 `TextRunEditController` 的 `TextEditDomController`，以及 `VectorEditController`。`SceneReconciler` 唯一持有 exact-revision projection 与 `projectionId → Leafer element` identity map，负责增量 add/update/tag replace/reparent/order/remove、Editor stale item 移除、warning 发布、Text data materialize、选区 bounds 刷新与 map dispose；adapter 和其他 owner 只持有窄查询/命令端口。跨 Page/Design Document、stale parent 或不连续 revision 会清理旧 interaction session，不能在新同步后的 Runtime 提交旧结果；stale transform 与 Vector drag 会恢复权威 projection。pointer event、Text DOM helper、Vector interaction 和 scene reconcile 已进入各自内部 owner，所有临时投影、DOM 内容、marker 与 Leafer element 仍未越过 `@opendesign/leafer-engine` 或进入文档/history。其余 generation presentation、scheduler 和 geometry/resource lifecycle 仍留在 adapter，SVG 与 release inventory 也保持开放。

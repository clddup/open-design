...
步骤 8 的 Leafer 阶段已开始但未完成：七个 interaction controller 已分别完整持有 Image Crop、Box Draw、Pen、Box Select、直接变换、Text DOM 与 Vector Edit 生命周期；`SceneReconciler` 唯一持有 exact-revision projection 与 `projectionId → Leafer element` identity map。`GenerationPresentationController` 现唯一持有 plan skeleton、已提交 revision 的 reveal/tween、Agent activity cursor、focus point、processed/suppressed ID、reduced-motion、viewport alignment、frame cadence/scheduler 与 layer/stroker 生命周期，并只通过 `SceneReconciler` 读取当前 scene。`SceneProjectionController` 唯一持有基础 Page mapping、增量 projection、Boolean/Text/Edit-scope 派生投影，以及 vector geometry provider 的按需加载、失败、retry、迟到结果拒绝和 dispose。base/projection 只有在 live scene reconcile 成功后才提交；失败会恢复权威投影，并让同一 revision 的下一次 sync 强制 full rebuild/reapply。geometry wait 可被 dispose 结束，异步 Boolean reconcile 失败可重试。`LeaferFrameScheduler` 唯一合并 viewport/editor animation frame、tree/node bounds refresh，并以自身 terminal dispose 取消待执行资源。capture/export/direct interaction、identity change、失败和 dispose 会先恢复最终权威 opacity/transform，再取消 animation frame 和销毁资源；skeleton/cursor/reveal 不进入 document、revision、history 或 delivery ledger。adapter 只组合窄端口并路由 Leafer/InnerEditor/viewport 事件。监听器与 hover chrome 等剩余宿主资源仍待迁移，SVG 与 release inventory 也保持开放。

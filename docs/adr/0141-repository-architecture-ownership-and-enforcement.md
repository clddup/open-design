...
步骤 8 的 Leafer 阶段已开始但未完成：七个 interaction controller 已分别完整持有 Image Crop、Box Draw、Pen、Box Select、直接变换、Text DOM 与 Vector Edit 生命周期；`SceneReconciler` 唯一持有 exact-revision projection 与 `projectionId → Leafer element` identity map。`GenerationPresentationController` 现唯一持有 plan skeleton、已提交 revision 的 reveal/tween、Agent activity cursor、focus point、processed/suppressed ID、reduced-motion、viewport alignment、frame cadence/scheduler 与 layer/stroker 生命周期，并只通过 `SceneReconciler` 读取当前 scene。capture/export/direct interaction、identity change、失败和 dispose 会先恢复最终权威 opacity/transform，再取消 animation frame 和销毁资源；skeleton/cursor/reveal 不进入 document、revision、history 或 delivery ledger。adapter 只组合窄端口并路由 Leafer/InnerEditor/viewport 事件。其余 Boolean geometry projection/provider 与更细资源生命周期仍留在 adapter，SVG 与 release inventory 也保持开放。

...
1. `createDesignVisualCriticVerdictContract()` 根据当前 criterion IDs 与 draft/final phase 构造一次动态可执行 Schema；同一 Schema 对象同时用于 Provider tool disclosure 与 Runtime parse。
2. Schema 直接拥有 summary、完整动态 criteria map、score/evidence/refinement 和 draft refinement 数量；删除并行的手写 parser、字段枚举和错误解释。
3. Capture structured content 与 JPEG attachment 分别使用可执行 Contract；attachment 继续拒绝额外路径字段和非 JPEG MIME。
4. 非法 Critic response 进入现有 `visual_critic_unavailable` workflow failure，首个准确 Contract path 与恢复建议进入结构化 failure，不再只返回 `invalid scorecard` 字符串。
5. 不增加 Critic 次数、Provider retry、固定等待、兼容 parser 或新的公开设计工具。

## 结果

- Provider 与 Runtime 不再维护两套 Critic verdict 结构。
- 动态 Logo direction criterion 的错误可定位到 `/criteria/<criterionId>/<field>`。
- draft 缺少 refinement、分数越界、criterion 缺失或多余字段在同一 Contract 中得到一致结果。
...
- draft refinement 数量不足返回 `/refinements`。

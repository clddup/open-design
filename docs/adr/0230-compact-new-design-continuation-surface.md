...
3. continuation 可以从同一权威定义投影阶段专用的 Provider 描述和 Schema，但执行 definition、Runtime validator、Main 路由和 EditorRuntime 不变。当前普通编辑投影保留基础节点写入，并加入 align、distribute、spacing、`repair-overflow` 与 `resize-frame`，避免为了缩小协议反而失去确定性布局修复；设计系统投影只保留 Component Main 与 Instance 创建，不把 Variable、Style、Variant 和实例高级编辑提前塞入当前阶段。
4. Vector、Text Range、Font、Image Update/Edit、SVG Import/Export、Raster Export、Page access/lifecycle、Capabilities 和 legacy Visual Review 不进入新建设计 continuation；它们仍存在于完整 catalog，并在 general Run 或明确的后续编辑 Run 中可用。
5. `surfaces` 只控制 Provider 看到的定义，不改变执行注册、权限、schema、Main 路由、EditorRuntime 事务或历史事实。未声明 `surfaces` 的工具保持 general-only，避免隐式把高级能力带入紧凑流程。
6. 不以“必须是 X 个工具”、schema hash、源码数量或 fixture 数量作为质量门禁。验证只证明阶段语义：当前阶段入口存在，高级互斥入口未被无条件披露；真实 Provider turn、首个 revision 时间和历史修改成功率继续用打包产品样本评估。
...
- continuation 的静态 Provider 定义约为 53 KB，其中统一编辑仍能执行确定性布局修复，设计系统只披露两个组件 authoring action；该数字是审计结果，不是构建或发布门禁。
...
- 同一 catalog 回归验证 continuation 的统一编辑保留布局修复分支、设计系统只投影 Component Main/Instance 创建；

...
通用 node apply 的 compact DSL 已补齐“模型契约合法输入 → canonical operation”的确定性编译边界。Insert/Replace 的 `kind` 分支在 Provider Schema 阶段要求 Line、Polygon、Star、Text、Image 与 Path/Vector 的真实语义字段；node kind 与 property key、Paint type 与 field、Path/Vector 双 geometry source 属于一个可见说明加单一跨字段 model refinement。只有结构与跨字段关系均合法后才运行 appearance/export compiler；`fills/strokes=[]`、`strokeWidth/cornerRadius=0`、Frame `clipsContent=false` 和完整 Export defaults 是公开 DSL 的确定性输出，不是非法输入修复。编译结果由同一 `validateContract` canonical phase 对完整 `DesignOperationSchema` 做宿主 invariant 断言；真实 node/Paint 全分支 fixtures 证明 Provider 合法样本不会再以 `canonical_invalid` 要求模型重试。Update patch 因模型只持有 inspected nodeId 而不回抄 kind，字段是否属于当前节点仍由 Main 合并当前节点后验证，这是 current-document guard，不是第二份输入结构 Schema。
...
通用 node apply 已完成第二个迁移切片：`DesignApplyContract.parse(input, context)` 明确区分模型 compact 输入、Main/Renderer canonical 输入与 trusted internal 操作，但三种阶段共享同一个 `validateContract` 入口和结构化 issue。模型可见 JSON Schema 被原样赋予不可序列化的 TypeBox runtime metadata，Provider JSON 与 Runtime schema 不再复制；compact node/Paint 分支先在模型结构与 model refinement 阶段给出准确字段路径，再由 total compiler 生成完整 canonical node/export defaults。旧 `normalizeDesignApplyToolInput / isDesignApplyToolInput / isInternalDesignApplyToolInput / explainInvalidDesignApplyToolInput` 及独立 `canonicalCommandIssues` 已删除。语义 step 顺序、内部字段、允许的 operation、Component Instance 边界与 rebase target 唯一性只在一个 canonical refinement 中维护；Pi 直接消费 `validateInputIssues`，不再回退到字符串 explainer。
...
可执行 schema 的组合本身也属于契约基础设施：已经由 `executableJsonSchema()` 装饰的子 schema 再嵌入 Checkpoint 等父 schema 时，空 object 的 TypeBox `properties` metadata 必须在新 clone 上恢复，不能因为源对象持有 non-enumerable metadata 就跳过。生产中的完整 `replace_subtree`（含 `extensions: {}` 与 Text `maxLines: null`）曾因此在合法输入上抛出 `Cannot convert undefined or null to object`；现由 schema decorator 根修复并以真实组合形状回归，不在 Checkpoint handler 增加特判。
...
普通 Node、Hierarchy 与 Arrange 已进一步合并为一个 `opendesign_edit_design` 模型入口；旧独立公开名称和 Renderer 直达分支已删除。其他 Agent tools、IPC 与持久化边界仍按各自 owner 分阶段审计，不得据此宣称全仓所有输入都已完成迁移。

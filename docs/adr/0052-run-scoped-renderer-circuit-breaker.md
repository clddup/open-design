...
第一次仍返回原有可恢复 `renderer_idle_timeout`。离屏导出自身的有界 `renderer_capture_timeout` 也计入同一 Run 的连续画布停滞；同一 Run 第二次连续停滞打开 circuit，并返回：
...
- 相同 Renderer 故障最多再浪费一次有界等待，不再持续数十轮或由自动 continuation 重启循环。离屏 Leafer 的 `waitViewCompleted` 最多等待 2 秒；隐藏 surface 不回调时，capture 继续用同步 export 直接计算和渲染目标树，而不是等待外层 30 秒 deadline。
- 已有可用页面和已提交步骤保持可编辑、可撤销和可保存。
- 新 Run 使用新的 circuit scope；产品文案仍建议重启，是因为本切片没有证明底层 Renderer/GPU/export 状态能在原进程内恢复。
- 本决策同时修复已确认的隐藏离屏 surface readiness 挂起；其他 Leafer capture/apply 停滞仍需按实机证据定位，也不改变正常 Provider 串行往返造成的生成时长。1/4/12 target 性能样本继续作为独立工作。
...
- 两次 `renderer_capture_timeout` 即使中间成功 inspect 也打开 circuit；
- `waitViewCompleted` 永不回调时在 2 秒后执行一次同步 JPEG export，健康 surface 仍立即导出且不同 surface 互不排队；

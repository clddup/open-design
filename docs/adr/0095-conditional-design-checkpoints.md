...
- 文档协议：不变
- 关联：ADR-0018、ADR-0050、ADR-0052、ADR-0072、ADR-0080、ADR-0117
...
2. `refine-and-capture`：只在上一 clean capture 已由 ADR-0117 的独立 Critic 自动形成可信 review 后接收一笔 canonical refinement；refinement 成功并返回新 revision 后，才执行 final capture。作者不再重复提交 review。

工具不跨越 approval、inspection、图片生成/读取、Page lifecycle 或任何后一步必须读取前一步未知结果的边界。模型输入继续经过原 apply validator；复合工具不放宽节点、Plan、active target、brief fidelity、layout quality 或 independent-critic 门禁。

### 复用唯一执行路径

Main 的普通 apply 与 capture 路由和 checkpoint 子阶段调用同一组执行函数：
...
- capture 的独立 Critic 继续只由 Main 发起并由 Coordinator 登记，不把作者模型文本当作渲染或完成证明。
...
- apply 失败时不 capture；缺少上一 capture 的可信 review 时 refinement 会被 Coordinator 拒绝；refinement 失败时不 capture。
...
- 首个真实切片不再等待模型专门发起首次截图；已知的 apply→capture 和 independent review→refine→capture 各删除纯控制型 Provider 往返。

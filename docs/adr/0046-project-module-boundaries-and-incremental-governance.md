...
Phase 8 已落地七个 Leafer interaction owner、scene reconcile、generation presentation、scene projection 和 frame scheduler owner。Image Crop、Box Draw、Pen、Box Select、直接变换、Text DOM 与 Vector Edit 的完整 session、preview/overlay、pointer/key、scope/revision、提交/拒绝恢复、取消和 dispose 分别由独立 controller 单一持有；`SceneReconciler` 唯一持有 exact-revision `LeaferSceneProjection` 与 `projectionId → Leafer element` identity map。`GenerationPresentationController` 持有计划/提交后的展示资源，`SceneProjectionController` 持有基础 Page、Boolean/Text/Edit-scope 派生投影和异步 geometry provider，`LeaferFrameScheduler` 持有合并帧与取消。adapter 只路由事件和组合窄端口。上述切片不改变 DesignDocument 或公共 adapter 契约，也不把 preview、DOM、marker、generation presentation 或 Leafer element 持久化；监听器与 hover chrome 等资源仍在后续切片中迁移，因此 Phase 8 保持进行中。

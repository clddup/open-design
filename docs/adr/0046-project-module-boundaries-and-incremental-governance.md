...
Phase 8 已落地七个 Leafer interaction owner、一个 scene reconcile owner 和一个 generation presentation owner。Image Crop、Box Draw、Pen、Box Select、直接变换、Text DOM 与 Vector Edit 的完整 session、preview/overlay、pointer/key、scope/revision、提交/拒绝恢复、取消和 dispose 分别由独立 controller 单一持有；`SceneReconciler` 唯一持有 exact-revision `LeaferSceneProjection` 与 `projectionId → Leafer element` identity map，并负责增量 add/update/tag replace/reparent/order/remove、Editor stale item 移除、warning、Text data materialize、选区 bounds 刷新与 map dispose。`GenerationPresentationController` 唯一持有 plan skeleton、已提交 scene revision 的 reveal/tween、Agent activity cursor、focus point、processed/suppressed ID、reduced-motion、viewport alignment、frame cadence/scheduler 和 layer/stroker 生命周期；它通过只读 `SceneReconciler` 端口投影，不写 document、revision、history 或 delivery ledger。背景 skeleton 和前景 activity/reveal 继续按原顺序挂载；capture/export/direct interaction、identity change、失败与 dispose 都先恢复 scene 的最终 opacity/transform，再取消 frame 并释放资源。adapter 只路由事件和组合窄端口。上述切片不改变 DesignDocument 或公共 adapter 契约，也不把 preview、DOM、marker、generation presentation 或 Leafer element 持久化。其余 Boolean geometry projection/provider 与资源生命周期仍在后续切片中迁移，因此 Phase 8 保持进行中。

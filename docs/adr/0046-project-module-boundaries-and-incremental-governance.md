...
Phase 8 已落地七个 Leafer interaction owner。Image Crop 的 session、overlay、pointer/viewport、完成/取消、stale 恢复、状态发布和清理由 `ImageCropController` 单一持有；Box Draw 的容器解析、screen/local 坐标、preview、drag/click 语义、Shift/Alt 约束、Line 默认几何、创建请求、Escape/tool switch/stale parent/revision、事务拒绝恢复与 dispose 由 `BoxDrawController` 单一持有；Pen 的 document/Page/revision scope、parent 解析、click/drag Bézier draft、zoom-stable preview、闭合候选、键盘编辑、tool-switch 延迟提交、network 规范化、拒绝恢复与 dispose 由 `PenToolController` 单一持有；Box Select 的初始 selection snapshot、screen/selector 坐标、pointer-up hit 重算、Shift toggle、最小拖拽阈值、Escape/pointer-cancel 恢复、tool/scope/revision 与 dispose 由 `BoxSelectController` 单一持有；直接变换的 selection/subtree snapshot、move/resize/rotate/skew 分类、Line endpoint 规范化、Boolean preview 调度、no-op、单次 typed commit、Escape/tool/selection/stale/reject/dispose 恢复由 `DirectTransformController` 单一持有；Text DOM 的 edit-root identity、attach/detach、UTF-16 selection、IME composition、typing-style marker、styled span 原位更新、自动列表、立即 Undo、列表快捷键、错误取消和 listener 清理由 `TextEditDomController` 单一持有，并组合现有唯一 `TextRunEditController`，不复制 rich-text session；Vector Edit 的多节点 scope、point/segment selection、anchor/handle/selection overlay、lasso、vertex/handle drag、document-space move/resize/rotate、Space reposition、point mode、path/line Cut、删除、键盘退出、拒绝恢复、stale revision 和 dispose 由 `VectorEditController` 单一持有。旧实现会在 Page/Design Document 切换与退出 Pen 同时发生时把旧 pending path 提交到新同步后的 Runtime，也会在非连续 revision 取消 transform 或 Vector drag 后因 projection spec 未变而残留 Leafer 临时几何。新 owner 在提交前复核稳定 scope/revision，并对 stale transform/Vector preview 主动恢复权威 projection。adapter 只路由 Leafer/InnerEditor 事件并提供 projection、element、tool、bounds refresh、Boolean preview 和 transaction callback 端口。七个切片都不改变 DesignDocument 或公共 adapter 契约，也不把交互 preview、DOM 内容或 marker 持久化。其余 Leafer reconcile、generation presentation、scheduler 与 geometry/resource owner 仍在后续切片中迁移，因此 Phase 8 保持进行中。

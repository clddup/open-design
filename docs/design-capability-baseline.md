...
| 基础图形          | Frame、Group、Rectangle、Ellipse、Line/Arrow、Polygon、Star、自由 Path/Vector、Slice/导出区域                                                              | `DesignDocument 1.9.0` 已提供正式 Line/Arrow、Polygon/Star、精确 SVG path-data、editable Vector Network 与已有单轮廓节点编辑；Line/Arrow 使用 Leafer Arrow/LineEditTool，Polygon/Star 使用 Leafer 原生节点，Path/Vector 统一进入 Leafer Path、PathKit Boolean 与受控 SVG。折线 connector、路由/吸附、Figma 式 corner smoothing、Slice、像素基线和双平台打包交互证据仍待完成                                                                                                                                      |
...
| 外观与合成        | 多 fill/stroke、纯色、线性/径向/角度渐变、图片填充、透明度、blend、描边位置/cap/join/dash、圆角、阴影、内阴影、光晕、模糊、灰度、蒙版、clip                | `DesignDocument 1.9.0`、EditorRuntime、属性面板和 Leafer 映射已实现主要语义；Line/Arrow、Polygon/Star、path-data 与 editable network 复用同一外观。SVG v1 已可确定性往返受控 Line marker、尖角规则图形、editable network、圆角 Frame clipping、四种 ordered sibling mask、最多八个普通零 spread 投影、一层 layer blur、效果顺序与可见性，并明确报告其余 filter/mask 语义；复杂组合的视觉与完整导出保真仍待实机/基线验证                                                                                          |
| 颜色系统          | Color picker、HEX/RGB/HSL/alpha、渐变编辑器、吸色、最近色、共享颜色样式、变量绑定、色彩空间与无障碍对比                                                    | 基础颜色输入和渐变 stop 已实现；专业 picker、吸色、共享样式/变量、色彩空间待实现                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 文字              | 文本框、自动宽/高、基础字体属性、段落对齐、文字内编辑、富文本 runs、列表、缩进、装饰、大小写、OpenType/variable font、文本样式、缺失字体替换、text-on-path | `DesignDocument 1.9.0` 已实现固定文字框、单样式、横纵对齐、`none/word/character` 换行、`visible/clip/ellipsis` 溢出、Leafer TextEditor、人工 Inspector、Agent schema 与 SVG metadata v2 往返；Auto Width/Auto Height、max-lines、Text/Font shaping service、字体 asset/替换、富文本与双平台视觉验收仍待实现                                                                                                                                                                                                      |
| 图片              | 选择/粘贴/拖放/路径/URL 导入、AI 生成与编辑、内容寻址 asset、来源/派生关系、嵌入画布、fit、crop、焦点、蒙版、替换、滤镜、调整、透明背景、大图资源生命周期  | attachment、`read_image`、独立应用级生图配置、GPT Image 2 `generate_image`、事务化 asset/image node、`place_image` 已实现；`DesignDocument 1.9.0` 与 Image service 已实现 Image 节点的非破坏 placement、crop/focal 几何和 Leafer 投影，检查器与 Agent 共用 placement 和来源替换 planner；画布 Crop 交互、调整滤镜、AI 局部重绘/扩图/背景替换和派生 asset 来源链仍待实现                                                                                                                                          |
...
`DesignDocument 1.9.0` 已用完整垂直切片加入正式 Line/Arrow、Polygon/Star、editable Vector Network / Pen 创作、已有单轮廓节点编辑，以及固定文字框换行/溢出，而不是为单个 UI 控件增加私有字段。后续文档协议升级仍必须围绕完整的专业语义切片，并至少统一设计和迁移以下剩余内容：
...
3. Text/Font auto-size 与确定性 shaping、max-lines、富文本 runs、paragraph、font axis、text style 与字体引用。

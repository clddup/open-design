...
| 导入              | OpenDesign native、SVG、PNG/JPEG/WebP/GIF、PDF、剪贴板矢量/位图；后续按明确需求评估 Figma/Sketch                                                           | native 文档和 Agent 图片附件链部分实现；`@opendesign/import-export-service` 已建立安全、版本化 SVG 结构导入 contract，可返回可编辑候选节点和显式 fidelity report，但尚未接 Main 文件桥、EditorRuntime planner、人工 UI 或 Agent 工具；其他设计格式仍待实现                                                                                                                                          |

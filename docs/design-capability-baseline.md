...
| 基础图形          | Frame、Group、Rectangle、Ellipse、Line/Arrow、Polygon、Star、自由 Path/Vector、Slice/导出区域                                                              | Frame/Group/Rectangle/Ellipse、正式 Line/Arrow 与 Path 渲染已存在；Line/Arrow 使用 `DesignDocument 1.5.0`、Leafer Arrow/LineEditTool、人工工具栏/快捷键/Inspector、Agent transaction 与受控 SVG marker，当前仍缺折线 connector、路由/吸附、像素基线和双平台打包交互证据；Polygon、Star 与 Slice 待实现                                                                                                           |
...
| 外观与合成        | 多 fill/stroke、纯色、线性/径向/角度渐变、图片填充、透明度、blend、描边位置/cap/join/dash、圆角、阴影、内阴影、光晕、模糊、灰度、蒙版、clip                | `DesignDocument 1.5.0`、EditorRuntime、属性面板和 Leafer 映射已实现主要语义；Line/Arrow 还提供独立端点装饰与开放描边约束；SVG v1 已可确定性往返受控 Line marker、圆角 Frame clipping、四种 ordered sibling mask、最多八个普通零 spread 投影、一层 layer blur、效果顺序与可见性，并明确报告其余 filter/mask 语义；复杂组合的视觉与完整导出保真仍待实机/基线验证                                                   |
...
| 图片              | 选择/粘贴/拖放/路径/URL 导入、AI 生成与编辑、内容寻址 asset、来源/派生关系、嵌入画布、fit、crop、焦点、蒙版、替换、滤镜、调整、透明背景、大图资源生命周期  | attachment、`read_image`、独立应用级生图配置、GPT Image 2 `generate_image`、事务化 asset/image node、`place_image` 已实现；`DesignDocument 1.5.0` 与 Image service 已实现 Image 节点的非破坏 placement、crop/focal 几何和 Leafer 投影；画布 Crop 交互、检查器、replace、调整、AI 局部重绘/扩图/背景替换和派生 asset 来源链待实现                                                                                 |
...
`DesignDocument 1.5.0` 已用一个完整垂直切片加入正式 Line/Arrow，而不是为单个 UI 控件增加私有字段。后续文档协议升级仍必须围绕完整的专业语义切片，并至少统一设计和迁移以下剩余内容：

1. 正式 Polygon/Star/Slice 节点、Line 后续 connector/routing 语义，以及 Path/Vector 多轮廓与编辑语义；不得退回无类型 `JsonObject`。

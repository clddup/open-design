...
| 基础图形          | Frame、Group、Rectangle、Ellipse、Line/Arrow、Polygon、Star、自由 Path/Vector、Slice/导出区域                                                              | `DesignDocument 1.7.0` 已提供正式 Line/Arrow、Polygon/Star、精确 SVG path-data 与 editable Vector Network；Line/Arrow 使用 Leafer Arrow/LineEditTool，Polygon/Star 使用 Leafer 原生节点，Path/Vector 统一进入 Leafer Path、PathKit Boolean 与受控 SVG。折线 connector、路由/吸附、Figma 式 corner smoothing、Slice、像素基线和双平台打包交互证据仍待完成                                                                |
| 矢量编辑          | Pen、节点/手柄编辑、开放/闭合路径、节点类型、路径反转、连接/断开、布尔 union/subtract/intersect/exclude、flatten、outline stroke                           | 已固定 PathKit geometry provider并打通非破坏 Boolean；`1.7.0` Pen 已支持 click 放点、drag 镜像 cubic handles、首点闭合、Enter/Escape 开放完成、Backspace 回退、切换工具收尾、tight bounds、单事务、Agent network schema 与受控 SVG network 往返。已有路径节点/手柄编辑、节点类型、分支/多轮廓、connect/disconnect、reverse、flatten、outline stroke、像素和双平台证据仍待完成                                           |
...
| 外观与合成        | 多 fill/stroke、纯色、线性/径向/角度渐变、图片填充、透明度、blend、描边位置/cap/join/dash、圆角、阴影、内阴影、光晕、模糊、灰度、蒙版、clip                | `DesignDocument 1.7.0`、EditorRuntime、属性面板和 Leafer 映射已实现主要语义；Line/Arrow、Polygon/Star、path-data 与 editable network 复用同一外观。SVG v1 已可确定性往返受控 Line marker、尖角规则图形、editable network、圆角 Frame clipping、四种 ordered sibling mask、最多八个普通零 spread 投影、一层 layer blur、效果顺序与可见性，并明确报告其余 filter/mask 语义；复杂组合的视觉与完整导出保真仍待实机/基线验证 |
...
| 图片              | 选择/粘贴/拖放/路径/URL 导入、AI 生成与编辑、内容寻址 asset、来源/派生关系、嵌入画布、fit、crop、焦点、蒙版、替换、滤镜、调整、透明背景、大图资源生命周期  | attachment、`read_image`、独立应用级生图配置、GPT Image 2 `generate_image`、事务化 asset/image node、`place_image` 已实现；`DesignDocument 1.7.0` 与 Image service 已实现 Image 节点的非破坏 placement、crop/focal 几何和 Leafer 投影；画布 Crop 交互、检查器、replace、调整、AI 局部重绘/扩图/背景替换和派生 asset 来源链待实现                                                                                        |
...
`DesignDocument 1.7.0` 已用完整垂直切片加入正式 Line/Arrow、Polygon/Star 与 editable Vector Network / Pen 创作，而不是为单个 UI 控件增加私有字段。后续文档协议升级仍必须围绕完整的专业语义切片，并至少统一设计和迁移以下剩余内容：

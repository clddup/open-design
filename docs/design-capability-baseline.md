...
| 导出与交付        | PNG/JPEG/WebP/SVG/PDF、多倍图、选区/Frame/批量导出、透明背景、颜色配置、token、开发检查、资源切图                                                          | SVG service 已能纯函数导出 Path/Vector/基础图形和显式解析的 Boolean result，并明确报告 Boolean flatten 与未保真特性；EditorRuntime planner 已支持显式 roots/Frame、base revision、world transform、0-origin viewport、paint order、padding、ID 和 Boolean snapshot 绑定。尚无应用内保存入口，PNG/JPEG/WebP/PDF、outline text/stroke、持久 export settings 与完整产品路径仍待实现，不能把画布截图当专业导出 |

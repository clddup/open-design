...
| 导出与交付        | PNG/JPEG/WebP/SVG/PDF、多倍图、选区/Frame/批量导出、透明背景、颜色配置、token、开发检查、资源切图                                                          | SVG service、EditorRuntime planner、人工原生保存入口与 Agent typed export 已贯通显式 roots/Frame、revision、world transform、0-origin viewport、paint order、padding、ID、Boolean snapshot、Frame clipping、标准 `<text>/<tspan>` + 受控 TextNode 往返、ordered sibling masks 和基础 filter effects。PNG/JPEG/WebP/PDF、可选 outline text/stroke、普通第三方 Text 的字体布局、持久 export settings、复杂组合 mask/image fidelity、双平台实机与完整专业交付仍待实现；画布截图不算交付产物      |
